...
func TestAllowNonFloatingPointSC(t *testing.T) {
	...
	txProc, accnts, blockchainHook := vm.CreatePreparedTxProcessorAndAccountsWithVMs(t, ownerNonce, ownerAddressBytes, ownerBalance)
	_, _ = blockchainHook.NewAddress(ownerAddressBytes, ownerNonce, factory.ArwenVirtualMachine)
...
func TestSCAbortExecution(t *testing.T) {
	...
	txNonAbort := &transaction.Transaction{
		Nonce:    ownerNonce + 1,
		Value:    big.NewInt(0),
		SndAddr:  ownerAddressBytes,
		RcvAddr:  scAddress,
	...
	err = txProc.ProcessTransaction(txNonAbort, round)

	txAbort := &transaction.Transaction{
		Nonce:    ownerNonce + 1,
		Value:    big.NewInt(0),
		SndAddr:  ownerAddressBytes,
		RcvAddr:  scAddress,
	...
	err = txProc.ProcessTransaction(txAbort, round)
}
