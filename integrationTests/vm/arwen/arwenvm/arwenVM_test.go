...
func TestAndCatchTrieError(t *testing.T) {
	...
	for i := 0; i < 51; i++ {
		...
		if i%10 == 5 {
			testContext.Accounts.PruneTrie(extraNewRootHash, state.NewRoot, state.NewPruningHandler(true))
		...
		ownerNonce++
		transferNonce++
		testContext.Accounts.PruneTrie(rootHash, state.OldRoot, state.NewPruningHandler(true))
		testContext.Accounts.PruneTrie(newRootHash, state.OldRoot, state.NewPruningHandler(true))
	}
}
