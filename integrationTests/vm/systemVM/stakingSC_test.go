...
func TestStakingUnstakingAndUnboundingOnMultiShardEnvironment(t *testing.T) {
	...
	for index, node := range nodes {
		...
		integrationTests.CreateAndSendTransaction(node, nodePrice, vm.AuctionSCAddress, txData)
	}
	...
	for index, node := range nodes {
		...
		integrationTests.CreateAndSendTransaction(node, big.NewInt(0), vm.AuctionSCAddress, txData)
	}
