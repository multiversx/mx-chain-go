...
func TestStakingUnstakingAndUnboundingOnMultiShardEnvironment(t *testing.T) {
	...
	for index, node := range nodes {
		...
		integrationTests.CreateAndSendTransactionOnTheCorrectShard(node, nodes, nodePrice, vm.AuctionSCAddress, txData, core.MinMetaTxExtraGasCost)
	}
	...
	for index, node := range nodes {
		...
		integrationTests.CreateAndSendTransactionOnTheCorrectShard(node, nodes, big.NewInt(0), vm.AuctionSCAddress, txData, core.MinMetaTxExtraGasCost)
	}
...
func TestStakeWithRewardsAddressAndValidatorStatistics(t *testing.T) {
	...
	for _, node := range nodes {
		...
		integrationTests.CreateAndSendTransaction(node, big.NewInt(0), vm.AuctionSCAddress, txData, core.MinMetaTxExtraGasCost)
	}
