...
func TestDynamicGasCostForDataTrieStorageLoad(t *testing.T) {
	...
	testContext, err := vm.CreatePreparedTxProcessorWithVMConfigWithShardCoordinatorDBAndGasAndRoundConfig(
		enableEpochs, shardCoordinator, integrationTests.CreateMemUnit(), gasScheduleNotifier, integrationTests.GetDefaultRoundsConfig(), vm.CreateVMConfigWithVersion("v1.4"))
	require.Nil(t, err)
