...
func TestHeadersAreResolvedByMetachainAndShard(t *testing.T) {
	...
	nodes[0].ShardDataPool.Headers().Add(shardHeaderHash, hdr)
	...
	for i := 0; i < numMetaNodes; i++ {
		nodes[i+1].MetaDataPool.Headers().Add(metaHeaderHash, metaHdr)
	}
	...
	for i := 0; i < numMetaNodes; i++ {
		nodes[i+1].MetaDataPool.Headers().Add(metaHeaderHash2, metaHdr2)
	}
