...
func TestChainSimulator_ExecuteOperationWithTransferDataForTokenWithPrefix(t *testing.T) {
	...
	// Deploy a contract for transfer and execute testing
	tcContractAddress := chainSim.DeployContract(t, cs, wallet.Bytes, &nonce, systemScAddress, "", saveCallerWasmPath)
	...
	// transfer and execute contract is the receiver and called on "pay" endpoint (just saves the tokens in balance and updates a storage with last caller)
	// original sender is wallet.Bytes
	executeMintOperation(t, cs, bridgeData.OwnerAccount.Wallet, tcContractAddress, &bridgeData.OwnerAccount.Nonce, bridgeData.ESDTSafeAddress, bridgedInTokens, wallet.Bytes, scTransferData)

	res, _, err := nodeHandler.GetFacadeHandler().ExecuteSCQuery(&process.SCQuery{
		ScAddress:  tcContractAddress,
	...
	require.Equal(t, wallet.Bytes, lastCaller) // EXPECTED
	//require.Equal(t, bridgeData.ESDTSafeAddress, lastCaller) // NOT EXPECTED
	//require.Equal(t, bridgeData.OwnerAccount.Wallet.Bytes, lastCaller) // NOT EXPECTED
}
