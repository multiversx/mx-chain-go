...
func TestChainSimulator_ExecuteSameNft2Times(t *testing.T) {
	...
	// 1st create success, 2nd create success, but 2nd create will override the 1st one's metadata (quantity and so on)
	// this means that 1 NFT will disappear
	// this operation will create 1 NFT and will stay in the bridge contract because transferAndExecuteByUser is not implemented and no "transfer back" event is implemented in the contract
	...
	for _, bridgedInToken := range groupTokens(bridgedInTokens) {
		chainSim.RequireAccountHasToken(t, cs, getTokenIdentifier(bridgedInToken), esdtSafeEncoded, big.NewInt(1))
		chainSim.RequireAccountHasToken(t, cs, getTokenIdentifier(bridgedInToken), wallet.Bech32, big.NewInt(0))
		requireMetadataInAccount(t, nodeHandler, bridgedInToken, esdtSafeEncoded, big.NewInt(1))
	}
}
