...
func transferAndCheckTokensMetaData(t *testing.T, isCrossShard bool, isMultiTransfer bool) {
	...
	addrs := vm2.CreateAddresses(t, cs, isCrossShard)

	err = cs.GenerateBlocksUntilEpochIsReached(int32(activationEpoch) - 1)
	require.Nil(t, err)

	vm2.Log.Info("Initial setup: Create NFT, SFT and metaESDT tokens (before the activation of DynamicEsdtFlag)")
	...
	tx = vm2.SetSpecialRoleTx(nonce, addrs[0].Bytes, addrs[1].Bytes, metaESDTTokenID, rolesTransfer)
	nonce++
	...
	tx = vm2.SetSpecialRoleTx(nonce, addrs[0].Bytes, addrs[1].Bytes, nftTokenID, rolesTransfer)
	nonce++
	...
	tx = vm2.SetSpecialRoleTx(nonce, addrs[0].Bytes, addrs[1].Bytes, sftTokenID, rolesTransfer)
	nonce++
	...
	vm2.Log.Info("Step 1. check that the metadata for all tokens is saved on the system account")
	...
	vm2.Log.Info("Step 3. transfer the tokens to another account")

	if isMultiTransfer {
		tx = vm2.MultiESDTNFTTransferTx(nonce, addrs[0].Bytes, addrs[1].Bytes, tokenIDs)
	...
	vm2.Log.Info("Step 4. check that the metadata for all tokens is saved on the system account")
	...
	vm2.Log.Info("Step 5. make an updateTokenID@tokenID function call on the ESDTSystem SC for all token types")
	...
	vm2.Log.Info("Step 6. check that the metadata for all tokens is saved on the system account")
	...
	vm2.Log.Info("Step 7. transfer the tokens to another account")

	nonce = uint64(0)
	if isMultiTransfer {
		tx = vm2.MultiESDTNFTTransferTx(nonce, addrs[1].Bytes, addrs[2].Bytes, tokenIDs)
		...
		for _, tokenID := range tokenIDs {
			...
			tx = vm2.EsdtNFTTransferTx(nonce, addrs[1].Bytes, addrs[2].Bytes, tokenID)
			txResult, err = cs.SendTxAndGenerateBlockTilTxIsExecuted(tx, vm2.MaxNumOfBlockToGenerateWhenExecutingTx)
			require.Nil(t, err)
			require.NotNil(t, txResult)
			require.Equal(t, "success", txResult.Status.String())
	...
	vm2.Log.Info("Step 8. check that the metaData for the NFT was removed from the system account and moved to the user account")
	...
	vm2.CheckMetaData(t, cs, addrs[2].Bytes, nftTokenID, shardID, nftMetaData)
	vm2.CheckMetaDataNotInAcc(t, cs, core.SystemAccountAddress, nftTokenID, shardID)

	vm2.Log.Info("Step 9. check that the metaData for the rest of the tokens is still present on the system account and not on the userAccount")

	vm2.CheckMetaData(t, cs, core.SystemAccountAddress, sftTokenID, shardID, sftMetaData)
	vm2.CheckMetaDataNotInAcc(t, cs, addrs[2].Bytes, sftTokenID, shardID)

	vm2.CheckMetaData(t, cs, core.SystemAccountAddress, metaESDTTokenID, shardID, esdtMetaData)
	vm2.CheckMetaDataNotInAcc(t, cs, addrs[2].Bytes, metaESDTTokenID, shardID)
}
...
func TestChainSimulator_ChangeMetaData(t *testing.T) {
	...
	t.Run("sft change metadata", func(t *testing.T) {
		testChainSimulatorChangeMetaData(t, vm2.IssueSemiFungibleTx)
	})

	t.Run("metaESDT change metadata", func(t *testing.T) {
		testChainSimulatorChangeMetaData(t, vm2.IssueMetaESDTTx)
	})
}

func testChainSimulatorChangeMetaData(t *testing.T, issueFn vm2.IssueTxFunc) {
	baseIssuingCost := "1000"
	...
	vm2.Log.Info("Initial setup: Create token and send in another shard")
	...
	vm2.SetAddressEsdtRoles(t, cs, nonce, addrs[1], tokenID, roles)
	nonce++

	vm2.Log.Info("Issued token id", "tokenID", string(tokenID))

	metaData := txsFee.GetDefaultMetaData()
	metaData.Nonce = []byte(hex.EncodeToString(big.NewInt(1).Bytes()))

	tx = vm2.EsdtNftCreateTx(nonce, addrs[1].Bytes, tokenID, metaData, 2)
	nonce++
	...
	tx = vm2.ChangeToDynamicTx(nonce, addrs[1].Bytes, tokenID)
	nonce++
	txResult, err = cs.SendTxAndGenerateBlockTilTxIsExecuted(tx, vm2.MaxNumOfBlockToGenerateWhenExecutingTx)
	require.Nil(t, err)
	require.NotNil(t, txResult)
	...
	vm2.Log.Info("Send to separate shards")
	...
	tx = vm2.EsdtNFTTransferTx(nonce, addrs[1].Bytes, addrs[0].Bytes, tokenID)
	nonce++
	txResult, err = cs.SendTxAndGenerateBlockTilTxIsExecuted(tx, vm2.MaxNumOfBlockToGenerateWhenExecutingTx)
	require.Nil(t, err)
	require.NotNil(t, txResult)
	...
	tx = vm2.SetSpecialRoleTx(nonce, addrs[1].Bytes, addrs[0].Bytes, tokenID, roles)
	...
	vm2.Log.Info("Step 1. change the sft meta data in one shard")
	...
	tx = vm2.EsdtMetaDataUpdateTx(tokenID, sftMetaData2, 0, addrs[0].Bytes)
	txResult, err = cs.SendTxAndGenerateBlockTilTxIsExecuted(tx, vm2.MaxNumOfBlockToGenerateWhenExecutingTx)
	require.Nil(t, err)
	require.NotNil(t, txResult)
	require.Equal(t, "success", txResult.Status.String())

	vm2.Log.Info("Step 2. check that the newest metadata is saved")

	shardID := cs.GetNodeHandler(0).GetProcessComponents().ShardCoordinator().ComputeId(addrs[0].Bytes)
	vm2.CheckMetaData(t, cs, core.SystemAccountAddress, tokenID, shardID, sftMetaData2)

	shard2ID := cs.GetNodeHandler(0).GetProcessComponents().ShardCoordinator().ComputeId(addrs[2].Bytes)
	vm2.CheckMetaData(t, cs, core.SystemAccountAddress, tokenID, shard2ID, metaData)

	vm2.Log.Info("Step 3. create new wallet is shard 2")
	...
	vm2.Log.Info("Step 4. send updated token to shard 2 ")

	tx = vm2.EsdtNFTTransferTx(1, addrs[0].Bytes, newShard2Addr.Bytes, tokenID)
	txResult, err = cs.SendTxAndGenerateBlockTilTxIsExecuted(tx, vm2.MaxNumOfBlockToGenerateWhenExecutingTx)
	require.Nil(t, err)
	require.NotNil(t, txResult)
	require.Equal(t, "success", txResult.Status.String())
	...
	vm2.Log.Info("Step 5. check meta data in shard 2 is updated to latest version ")

	vm2.CheckMetaData(t, cs, core.SystemAccountAddress, tokenID, shard2ID, sftMetaData2)
	vm2.CheckMetaData(t, cs, core.SystemAccountAddress, tokenID, shardID, sftMetaData2)
...
func TestChainSimulator_MetaESDT_RegisterAndSetAllRolesDynamic(t *testing.T) {
	...
	vm2.Log.Info("Register dynamic meta esdtImprovements token")
