...
// Internal test scenario #9
func TestChainSimulator_TopUpIsConsideredWhenUsersDelegateUndelegate(t *testing.T) {
	if testing.Short() {
	...
	// 1. Create 2 delegation contracts with different owners: A and B
	// 2. With each owner, add nodes
	// 3. With 2 delegators, delegate: delegatorA to A -> 1260 EGLD, delegatorB to B -> 1255 EGLD
	// 4. Stake nodes after delegation
	// // 5. Check if delegation contract: A is above B
	t.Run("staking ph 4 is not active", func(t *testing.T) {
		...
		testChainSimulatorTopUpIsConsideredWhenUsersDelegateUndelegate(t, cs, 1)
	})
	...
	t.Run("staking ph 4 step 1 is active", func(t *testing.T) {
		...
		testChainSimulatorTopUpIsConsideredWhenUsersDelegateUndelegate(t, cs, 2)
	})
	...
	// 2. With each owner, add nodes
	// 3. With 2 delegators, delegate: delegatorA to A -> 1260 EGLD, delegatorB to B -> 1255 EGLD
	// 4. delegation contract owners stake nodes after delegation
	// 5. Check if delegation contract: A is above B
	t.Run("staking ph 4 step 2 is active", func(t *testing.T) {
		...
		testChainSimulatorTopUpIsConsideredWhenUsersDelegateUndelegate(t, cs, 3)
	})
	...
	t.Run("staking ph 4 step 3 is active", func(t *testing.T) {
		...
		testChainSimulatorTopUpIsConsideredWhenUsersDelegateUndelegate(t, cs, 4)
	})
}
...
func testChainSimulatorTopUpIsConsideredWhenUsersDelegateUndelegate(t *testing.T, cs chainSimulatorIntegrationTests.ChainSimulator, targetEpoch int32) {
	...
	delegator1BytesShardID := cs.GetNodeHandler(0).GetShardCoordinator().ComputeId(delegator1Bytes)
	...
	delegator2BytesShardID := cs.GetNodeHandler(0).GetShardCoordinator().ComputeId(delegator2Bytes)
	...
	delegationA := big.NewInt(0).Mul(oneEGLD, big.NewInt(1260))
	delegationB := big.NewInt(0).Mul(oneEGLD, big.NewInt(1255))
	...
	if activationEpoch <= metachainNode.GetCoreComponents().EnableEpochsHandler().GetCurrentEpoch() {
		...
		for i, position := range auctionList {
			if strings.EqualFold(position.AuctionList[0].BlsKey, addedNodeBlsKeyA[0]) {
				contractAIndex = i
			}
			if strings.EqualFold(position.AuctionList[0].BlsKey, addedNodeBlsKeyB[0]) {
				contractBIndex = i
			}

			if contractAIndex != -1 && contractBIndex != -1 {
				break
			}
		}
		require.True(t, contractAIndex < contractBIndex)
	}
}
...
func createDelegationContract(t *testing.T, cs chainSimulatorIntegrationTests.ChainSimulator, validatorOwnerBytes []byte, nonce uint64, maxDelegationCap *big.Int, serviceFee *big.Int) []byte {
	txCreateDelegationContract := generateTransaction(validatorOwnerBytes, nonce, vm.DelegationManagerSCAddress, stakeValue,
	...
	delegationContractAddress, err := cs.GetNodeHandler(0).GetCoreComponents().AddressPubKeyConverter().Encode(delegationContractAddressHex)
	require.Nil(t, err)
	...
	found := false
	index := -1
	for i, addressBytes := range output.ReturnData {
		returnAddress, err := cs.GetNodeHandler(0).GetCoreComponents().AddressPubKeyConverter().Encode(addressBytes)
		require.Nil(t, err)

		if returnAddress == delegationContractAddress {
			found = true
			index = i
			break
		}
	...
	// Returning the delegationContractAddressBytes for further use
	return output.ReturnData[index]
}

// Sdd validator nodes to the delegation contract and verify the addition
func addValidatorNodeAndVerify(t *testing.T, cs chainSimulatorIntegrationTests.ChainSimulator, delegationContractAddressBytes, validatorOwnerBytes []byte, nonce uint64, metachainNode chainSimulatorProcess.NodeHandler) []string {
	// Step 1: Generate BLS keys and add validator keys
	...
	// Step 2: Prepare and send the "addNodes" transaction
	signatures := getSignatures(delegationContractAddressBytes, validatorSecretKeysBytes)
	txAddNodesData := addNodesTxData(blsKeys, signatures)
	txAddNodes := generateTransaction(validatorOwnerBytes, nonce, delegationContractAddressBytes, big.NewInt(0), txAddNodesData, gasLimitForAddNodesOperation)
	...
	// Step 3: Verify nodes are added successfully
...
func stakeNodesAndVerify(t *testing.T, cs chainSimulatorIntegrationTests.ChainSimulator, delegationContractAddressBytes, validatorOwnerBytes []byte, nonce uint64, blsKeys []string, expectedTopUp *big.Int, metachainNode chainSimulatorProcess.NodeHandler) {
	...
	// Adjust expected top-up value
	expectedTopUp = expectedTopUp.Sub(expectedTopUp, minimumStakeValue)
	// Verify top-up value adjustment
	actualTopUpValue := getBLSTopUpValue(t, metachainNode, delegationContractAddressBytes)
	require.Equal(t, expectedTopUp, actualTopUpValue)
...
// Delegate stake from a given delegator to a delegation contract and verify the state
func delegateAndVerify(t *testing.T, cs chainSimulatorIntegrationTests.ChainSimulator, delegationContractAddressBytes []byte,
	delegatorBytes []byte, nonce uint64, delegationValue *big.Int, expectedTopUp *big.Int, expectedTotalStaked *big.Int,
	metachainNode chainSimulatorProcess.NodeHandler) *big.Int {

	txDelegate := generateTransaction(delegatorBytes, nonce, delegationContractAddressBytes, delegationValue, "delegate", gasLimitForDelegate)
	delegateTx, err := cs.SendTxAndGenerateBlockTilTxIsExecuted(txDelegate, maxNumOfBlockToGenerateWhenExecutingTx)
	require.Nil(t, err)
	require.NotNil(t, delegateTx)

	// Update expected values
	newExpectedTopUp := expectedTopUp.Add(expectedTopUp, delegationValue)
	newExpectedTotalStaked := expectedTotalStaked.Add(expectedTotalStaked, delegationValue)

	// Verify total active stake
	output, err := executeQuery(cs, core.MetachainShardId, delegationContractAddressBytes, "getTotalActiveStake", nil)
	require.Nil(t, err)
	require.Equal(t, newExpectedTotalStaked, big.NewInt(0).SetBytes(output.ReturnData[0]))
	require.Equal(t, newExpectedTopUp, getBLSTopUpValue(t, metachainNode, delegationContractAddressBytes))

	// Verify user active stake
	output, err = executeQuery(cs, core.MetachainShardId, delegationContractAddressBytes, "getUserActiveStake", [][]byte{delegatorBytes})
	require.Nil(t, err)
	require.Equal(t, delegationValue, big.NewInt(0).SetBytes(output.ReturnData[0]))

	return newExpectedTopUp
}
