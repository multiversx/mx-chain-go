...
func TestChainSimulator_AddValidatorKey(t *testing.T) {
	...
	stake.Log.Info("balance before validator", "value", balanceBeforeActiveValidator)
	stake.Log.Info("balance after validator", "value", balanceAfterActiveValidator)
	...
	stake.Log.Info("difference", "value", diff.String())
...
func testChainSimulatorDirectStakedNodesStakingFunds(t *testing.T, cs chainSimulatorIntegrationTests.ChainSimulator, targetEpoch int32) {
	...
	stake.Log.Info("Preconditions. Have an account with 2 staked nodes")
	...
	stake.Log.Info("Step 2. Create from the owner of the staked nodes a tx to stake 1 EGLD")
	...
	stake.Log.Info("Step 3. Check the stake amount for the owner of the staked nodes")
	stake.CheckExpectedStakedValue(t, metachainNode, validatorOwner.Bytes, 5001)
}
