...
func TestPutNotPresent(t *testing.T) {
	...
	err := s.Put(key, val, common.TestPriority)

	assert.Nil(t, err, "no error expected but got %s", err)

	err = s.Has(key, common.TestPriority)
...
func TestPutPresentShouldOverwriteValue(t *testing.T) {
	...
	newVal := []byte("value5")
	err = s.Put(key, newVal, common.TestPriority)
	assert.Nil(t, err, "no error expected but got %s", err)

	returnedVal, err := s.Get(key, common.TestPriority)
	assert.Nil(t, err)
...
func TestGetNotPresent(t *testing.T) {
	key := []byte("key3")
	s := initStorageUnit(t, 10)
	v, err := s.Get(key, common.TestPriority)
...
func TestDeleteNotPresent(t *testing.T) {
	key := []byte("key12")
	s := initStorageUnit(t, 10)
	err := s.Remove(key, common.TestPriority)
...
func BenchmarkStorageUnit_Put(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		err := s.Put([]byte(strconv.Itoa(nr)), []byte(strconv.Itoa(nr)), common.TestPriority)
		logError(err)
	}
}
...
func BenchmarkStorageUnit_GetWithDataBeingPresent(b *testing.B) {
	...
	for i := 0; i < valuesInDb; i++ {
		err := s.Put([]byte(strconv.Itoa(i)), []byte(strconv.Itoa(i)), common.TestPriority)
		logError(err)
	}
	b.StartTimer()
	...
	for i := 0; i < b.N; i++ {
		...
		_, err := s.Get([]byte(strconv.Itoa(nr)), common.TestPriority)
		logError(err)
	}
}
