...
func isSCRForSenderWithRefund(scr *smartContractResult.SmartContractResult, txHash string, tx data.TransactionHandler) bool {
	isForSender := bytes.Equal(scr.RcvAddr, tx.GetSndAddr())
	isRightNonce := scr.Nonce == tx.GetNonce()+1
	isFromCurrentTx := hex.EncodeToString(scr.PrevTxHash) == txHash
