...
func testExtractAlteredAccountsFromPoolShouldIncludeESDT(t *testing.T) {
	...
	encodedAddr, err := args.AddressConverter.Encode([]byte("addr"))
	require.Nil(t, err)
	require.Len(t, res, 1)
...
func testExtractAlteredAccountsFromPoolESDTTransferBalanceNotChanged(t *testing.T) {
	...
	encodedAddrSnd, err := args.AddressConverter.Encode([]byte("snd"))
	require.Nil(t, err)
	encodedAddrRcv, err := args.AddressConverter.Encode([]byte("rcv"))
	require.Nil(t, err)
