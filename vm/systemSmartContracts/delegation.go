...
func (d *delegation) Execute(args *vmcommon.ContractCallInput) vmcommon.ReturnCode {
	...
	case "claimRewardsViaLiquidStaking":
		return d.claimRewardsViaLiquidStaking(args)
	case "reDelegateRewardsViaLiquidStaking":
		return d.reDelegateRewardsViaLiquidStaking(args)
	case "unDelegateViaLiquidStaking":
...
func (d *delegation) unDelegateValueFromAddress(
	...
	if isStakeLocked(d.eei, d.governanceSCAddr, delegatorAddress) {
...
func (d *delegation) basicCheckForLiquidStaking(args *vmcommon.ContractCallInput) vmcommon.ReturnCode {
	...
	if !bytes.Equal(args.CallerAddr, vm.LiquidStakingSCAddress) {
		d.eei.AddReturnMessage("only liquid staking sc can call this function")
		return vmcommon.UserError
	}
	if args.CallValue.Cmp(zero) != 0 {
		d.eei.AddReturnMessage("call value must be 0")
	...
	if value.Cmp(zero) <= 0 {
		d.eei.AddReturnMessage("invalid argument for value as bigInt")
		return vmcommon.UserError
	}
	if len(address) != len(d.validatorSCAddr) {
	...
	minDelegationAmount := delegationManagement.MinDelegationAmount
	belowMinDelegationAmount := value.Cmp(minDelegationAmount) < 0
	if belowMinDelegationAmount {
		d.eei.AddReturnMessage("call value below minimum to operate")
		return vmcommon.UserError
	}
...
func (d *delegation) claimDelegatedPosition(args *vmcommon.ContractCallInput) vmcommon.ReturnCode {
	returnCode := d.basicCheckForLiquidStaking(args)
	if returnCode != vmcommon.Ok {
		return returnCode
	}
	...
	isNew, delegator, err := d.getOrCreateDelegatorData(address)
	if err != nil {
		d.eei.AddReturnMessage(err.Error())
		return vmcommon.UserError
	}
	...
	if value.Cmp(activeFund.Value) > 0 {
		d.eei.AddReturnMessage("not enough funds to claim position")
		return vmcommon.UserError
	}
	...
	activeFund.Value.Sub(activeFund.Value, value)
	err = d.checkRemainingFundValue(activeFund.Value)
	if err != nil {
		d.eei.AddReturnMessage(err.Error())
		return vmcommon.UserError
	}
	...
	err = d.saveDelegatorData(address, delegator)
	if err != nil {
		d.eei.AddReturnMessage(err.Error())
		return vmcommon.UserError
	}
...
func (d *delegation) claimRewardsViaLiquidStaking(args *vmcommon.ContractCallInput) vmcommon.ReturnCode {
	returnCode := d.basicCheckForLiquidStaking(args)
	if returnCode != vmcommon.Ok {
		return returnCode
	}
	...
	d.eei.Transfer(address, args.RecipientAddr, totalRewards, nil, 0)
	return vmcommon.Ok
}
...
func (d *delegation) reDelegateRewardsViaLiquidStaking(args *vmcommon.ContractCallInput) vmcommon.ReturnCode {
	...
	if totalRewards.Cmp(zero) <= 0 {
		d.eei.AddReturnMessage("no rewards to redelegate via liquid staking")
		return vmcommon.UserError
	}

	dConfig, dStatus, globalFund, err := d.getConfigStatusAndGlobalFund()
	if err != nil {
		d.eei.AddReturnMessage(err.Error())
		return vmcommon.UserError
	}

	globalFund.TotalActive.Add(globalFund.TotalActive, totalRewards)
	withDelegationCap := dConfig.MaxDelegationCap.Cmp(zero) != 0
	if withDelegationCap && dConfig.CheckCapOnReDelegateRewards && globalFund.TotalActive.Cmp(dConfig.MaxDelegationCap) > 0 {
		d.eei.AddReturnMessage("total delegation cap reached")
		return vmcommon.UserError
	}

	returnCode = d.executeStakeAndUpdateStatus(dConfig, dStatus, globalFund, totalRewards, args.RecipientAddr)
	if returnCode != vmcommon.Ok {
		return returnCode
	}
...
func (d *delegation) returnViaLiquidStaking(args *vmcommon.ContractCallInput) vmcommon.ReturnCode {
	...
	value := big.NewInt(0).SetBytes(args.Arguments[1])
	checkPoint := uint32(big.NewInt(0).SetBytes(args.Arguments[2]).Uint64())
	rewardsFromPosition, err := d.computeRewards(checkPoint, false, value)
	if err != nil {
		d.eei.AddReturnMessage(err.Error())
		return vmcommon.UserError
	}
	...
	delegator.UnClaimedRewards.Add(delegator.UnClaimedRewards, rewardsFromPosition)
	err = d.addToActiveFund(address, delegator, value, dStatus, isNew)
	if err != nil {
		d.eei.AddReturnMessage(err.Error())
		return vmcommon.UserError
	}
