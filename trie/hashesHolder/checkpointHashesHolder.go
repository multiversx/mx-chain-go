...
type checkpointHashesHolder struct {
	hashes      []map[string]temporary.ModifiedHashes
	currentSize uint64
	maxSize     uint64
	hashSize    uint64
	mutex       sync.RWMutex
...
func NewCheckpointHashesHolder(maxSize uint64, hashSize uint64) *checkpointHashesHolder {
	...
	return &checkpointHashesHolder{
		hashes:      make([]map[string]temporary.ModifiedHashes, 0),
		currentSize: 0,
		maxSize:     maxSize,
		hashSize:    hashSize,
		mutex:       sync.RWMutex{},
...
func (c *checkpointHashesHolder) Put(rootHash []byte, hashes temporary.ModifiedHashes) bool {
	...
	newHashes := make(map[string]temporary.ModifiedHashes)
...
func (c *checkpointHashesHolder) removeHashFromMap(hash []byte, hashesMap map[string]temporary.ModifiedHashes) {
...
func isInMap(hash []byte, hashesMap map[string]temporary.ModifiedHashes) bool {
	for _, hashes := range hashesMap {
		_, ok := hashes[string(hash)]
		if ok {
			return true
...
func getMapSize(hashesMap map[string]temporary.ModifiedHashes, hashSize uint64) uint64 {
	mapSize := uint64(0)
