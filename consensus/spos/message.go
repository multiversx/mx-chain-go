...
func NewConsensusData(dta []byte, pks [][]byte, sig []byte, msg MessageType, ts []byte) *ConsensusData {
	return &ConsensusData{Data: dta, PubKeys: pks, Signature: sig, MsgType: msg, TimeStamp: ts}
}
...
func (msg *Message) SendCommitmentHash() bool {
	...
	dta := NewConsensusData(*msg.Cns.Data,
		nil,
		[]byte(msg.Cns.Self()),
		MtCommitmentHash,
		[]byte(msg.Cns.Chr.SyncTime().FormatedCurrentTime(msg.Cns.Chr.ClockOffset())))
...
func (msg *Message) ReceivedCommitmentHash(cnsDta *ConsensusData) bool {
	...
	if msg.Cns.Status(SrCommitmentHash) != SsFinished {
		if msg.Cns.IsNodeInValidationGroup(node) {
			if !msg.Cns.Validators.Agreement(node, SrCommitmentHash) {
				if msg.Cns.Data != nil {
					if bytes.Equal(cnsDta.Data, *msg.Cns.Data) {
...
func (msg *Message) ReceivedCommitment(cnsDta *ConsensusData) bool {
	...
	if msg.Cns.Status(SrCommitment) != SsFinished {
		if msg.Cns.IsNodeInBitmapGroup(node) {
			if !msg.Cns.Validators.Agreement(node, SrCommitment) {
				if msg.Cns.Data != nil {
					if bytes.Equal(cnsDta.Data, *msg.Cns.Data) {
...
func (msg *Message) ReceivedSignature(cnsDta *ConsensusData) bool {
	...
	if msg.Cns.Status(SrSignature) != SsFinished {
		if msg.Cns.IsNodeInBitmapGroup(node) {
			if !msg.Cns.Validators.Agreement(node, SrSignature) {
				if msg.Cns.Data != nil {
					if bytes.Equal(cnsDta.Data, *msg.Cns.Data) {
...
func (msg *Message) CheckIfBlockIsValid(receivedHeader *block.Header) bool {
	...
	if currentBlock == nil {
		...
		} else if receivedHeader.Nonce > 1 {
	...
	} else if receivedHeader.Nonce == currentBlock.Nonce+1 {
