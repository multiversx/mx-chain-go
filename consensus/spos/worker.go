...
func (wrk *Worker) ProcessReceivedMessage(message p2p.MessageP2P) error {
	...
	if wrk.consensusState.RoundIndex > cnsDta.RoundIndex {
		log.Info(fmt.Sprintf("wrk.consensusState.RoundIndex = %d and cnsDta.RoundIndex = %d\n",
			wrk.consensusState.RoundIndex, cnsDta.RoundIndex))
...
func (wrk *Worker) dysplaySignatureStatistic() {
	...
	if ok {
		for hash, pubKeys := range mapHashSigs {
			log.Info(fmt.Sprintf("in round %d, proposed header with hash %s has received signature from:\n",
				wrk.consensusState.RoundIndex,
				core.ToB64([]byte(hash))))

			for _, pubKey := range pubKeys {
				log.Info(fmt.Sprintf("%s", core.ToB64([]byte(pubKey))))
			}
		}
	}
