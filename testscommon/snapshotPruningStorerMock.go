...
func (spsm *SnapshotPruningStorerMock) GetFromOldEpochsWithoutAddingToCache(key []byte, _ int) ([]byte, error) {
	return spsm.Get(key)
}
...
func (spsm *SnapshotPruningStorerMock) GetFromEpochWithoutCache(key []byte, _ uint32) ([]byte, error) {
	return spsm.Get(key)
}
