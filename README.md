...
{FOLDER}/bootnode (binary generated by above commands)
{FOLDER}/config/config.testnet.json
{FOLDER}/genesis.json

With the text below could be created a start.sh file in the same 
{FOLDER} and run it. This is all! 

# Start 3 instances in a group of <consensusGroupSize> (genesis.json) from a total of <initialNodes> (genesis.json) validators,
# each of them having maximum -max-allowed-peers (flag) peers, with round duration of <roundDuration> (genesis.json) miliseconds 
