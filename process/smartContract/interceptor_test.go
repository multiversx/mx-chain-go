...
func TestTransactionInterceptor_ProcessReceivedMessageIntegrityFailedWithTwoScrsShouldErrAndFilter(t *testing.T) {
	t.Parallel()
	...
	scr1Buff, _ := marshalizer.Marshal(scr1)
	...
	scr2Buff, _ := marshalizer.Marshal(scr2)

	buff, _ := marshalizer.Marshal([][]byte{scr1Buff, scr2Buff})
	msg := &mock.P2PMessageMock{
		DataField: buff,
	}

	scri.SetBroadcastCallback(func(buffToSend []byte) {
		buff = buffToSend
	})
	err := scri.ProcessReceivedMessage(msg)

	assert.Equal(t, process.ErrNilSignature, err)
	//unmarshal data and check there is only scr2 inside
	scrBuffRecovered := make([][]byte, 0)
	_ = marshalizer.Unmarshal(&scrBuffRecovered, buff)
	assert.Equal(t, 1, len(scrBuffRecovered))
	scrRecovered := &smartContractResult.SmartContractResult{}
	_ = marshalizer.Unmarshal(scrRecovered, scrBuffRecovered[0])
	assert.Equal(t, scr2, scrRecovered)
}
