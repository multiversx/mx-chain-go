...
func createSovBlockchainHook() (process.BlockChainHookHandler, state.AccountsAdapter) {
	...
	storageManagerArgs.MainStorer = storer
	storageManagerArgs.CheckpointsStorer = storer

	trieFactoryManager, _ := trie.CreateTrieStorageManager(storageManagerArgs, storageMock.GetStorageManagerOptions())

	accountsDB := createAccountsDB(hasher, marshaller, accCreator, trieFactoryManager, &enableEpochsHandlerMock.EnableEpochsHandlerStub{})
	...
	args := hooks.ArgBlockChainHook{
		...
		NFTStorageHandler:     esdtDataStorage,
		GlobalSettingsHandler: &testscommon.ESDTGlobalSettingsHandlerStub{},
	...
	return sovBlockchainHook, accountsDB
}
...
func createRealSmartContractProcessorArguments() scrCommon.ArgsNewSmartContractProcessor {
	...
	sovBlock, accAdapter := createSovBlockchainHook()
