...
func (irp *intermediateResultsProcessor) AddIntermediateTransactions(txs []data.TransactionHandler) error {
	...
	for i := 0; i < len(txs); i++ {
		...
		irp.mapTxToResult[string(addScr.TxHash)] = append(irp.mapTxToResult[string(addScr.TxHash)], string(scrHash))
	}
...
func (irp *intermediateResultsProcessor) getShardIdsFromAddresses(sndAddr []byte, rcvAddr []byte) (uint32, uint32, error) {
	...
	isEmptyAddress := bytes.Equal(sndAddr, make([]byte, irp.pubkeyConv.AddressLen()))
	if isEmptyAddress {
		shardForSrc = irp.shardCoordinator.SelfId()
	}

	isEmptyAddress = bytes.Equal(rcvAddr, make([]byte, irp.pubkeyConv.AddressLen()))
	if isEmptyAddress {
