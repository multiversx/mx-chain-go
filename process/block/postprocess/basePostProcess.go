...
func (bpp *basePostProcessor) GetAllIntermediateTxsHashesForTxHash(txHash []byte) map[uint32][]string {
	bpp.mutInterResultsForBlock.RLock()
	defer bpp.mutInterResultsForBlock.RUnlock()

	mapIntermediateTxHashes := make(map[uint32][]string)
	...
	for _, intermediateTxHash := range intermediateTxHashes {
		...
		mapIntermediateTxHashes[txInfo.receiverShardID] = append(mapIntermediateTxHashes[txInfo.receiverShardID], intermediateTxHash)
	}
