...
// GetAllIntermediateTxsHashesForTxHash gets all the intermediate transaction hashes generated by the given transaction hash
func (bpp *basePostProcessor) GetAllIntermediateTxsHashesForTxHash(txHash []byte) map[uint32][][]byte {
	bpp.mutInterResultsForBlock.RLock()
	defer bpp.mutInterResultsForBlock.RUnlock()

	mapIntermediateTxHashes := make(map[uint32][][]byte)

	intermediateTxHashes, ok := bpp.mapTxToResult[string(txHash)]
	if !ok {
		log.Trace("basePostProcessor.GetAllIntermediateTxsHashesForTxHash: intermediate txs not found for the given tx hash", "tx hash", txHash)
		return mapIntermediateTxHashes
	}

	for _, intermediateTxHash := range intermediateTxHashes {
		txInfo, ok := bpp.interResultsForBlock[string(intermediateTxHash)]
		if !ok {
			log.Error("basePostProcessor.GetAllIntermediateTxsHashesForTxHash",
				"tx hash", txHash,
		...
		mapIntermediateTxHashes[txInfo.receiverShardID] = append(mapIntermediateTxHashes[txInfo.receiverShardID], []byte(intermediateTxHash))
	}
