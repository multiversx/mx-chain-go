...
func TestBaseProcessor_SetLastNotarizedExecutedBlockInfo(t *testing.T) {
	t.Parallel()

	t.Run("should fail is invalid last execution results handler", func(t *testing.T) {
		t.Parallel()
		...
		err := bp.SetLastNotarizedExecutedBlockInfo(header)
		require.Error(t, err)
	...
	t.Run("should work", func(t *testing.T) {
		...
		chain := &testscommon.ChainHandlerStub{
			SetLastExecutedBlockInfoCalled: func(nonce uint64, headerHash, rootHash []byte) {
				require.Equal(t, headerHash, headerHash)
