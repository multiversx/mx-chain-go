...
func TestGasConsumption_CheckTransactionsForMiniBlock(t *testing.T) {
	...
	require.Equal(t, uint32(9), lastIndex)
	...
	txsMb2 := getTxs(maxGasLimitPerTx, 10)
	txsMb3 := getTxs(maxGasLimitPerTx, 1)
	lastIndex, err = gc.CheckTransactionsForMiniBlock([]byte("mb1"), txsMb1)
	require.NoError(t, err)
	require.Equal(t, uint32(9), lastIndex) // added all
	...
	require.Equal(t, uint32(0), lastIndex) // limit reached, half of the limit for intra
	...
	require.NoError(t, err)
	require.Equal(t, uint32(9), lastIndex) // added all, different shard
...
func TestGasConsumption_DecreaseLimits(t *testing.T) {
	...
	// gas exceeded per mb
	txsToExceedMb := getTxs(maxGasLimitPerTx, 6) // 6 should be enough as the limit is nou 50 / mb
	lastIndex, err := gc.CheckTransactionsForMiniBlock([]byte("hash"), txsToExceedMb)
	require.Equal(t, process.ErrMaxGasLimitPerMiniBlockIsReached, err)
	require.Equal(t, uint32(4), lastIndex)
}
