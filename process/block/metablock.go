...
// ForceStart forces epoch start based on transactions to the validator SC
func (mp *metaProcessor) ForceStart(metaHdr *block.MetaBlock) {
	forceEpochTrigger, ok := mp.epochStartTrigger.(update.EpochHandler)
	if !ok {
		log.Debug("ForceStart: epochStartTrigger does not implement EpochHandler")
		return
	}
	...
	for _, tx := range txBlockTxs {
		if bytes.Equal(tx.GetRcvAddr(), vm.ValidatorSCAddress) {
...
func (mp *metaProcessor) epochsFastForward(metaHdr *block.MetaBlock, tokens []string) {
	...
	if err != nil {
		log.Error("epochfastforward", "epochs could not be parsed", tokens[1], "error", err)
		return
	}
	if epochs <= 0 {
		log.Error("epochfastforward", "epochs must be positive", tokens[1])
		return
	}

	roundsPerEpoch, err := strconv.ParseInt(tokens[2], 10, 64)
	if err != nil {
		log.Error("epochfastforward", "rounds could not be parsed", tokens[2], "error", err)
		return
	}
	if roundsPerEpoch <= 0 {
		log.Error("epochfastforward", "roundsPerEpoch must be positive", tokens[2])
		return
	}
	...
	log.Debug("epochfastforward - configured epoch fast forward", "round", metaHdr.GetRound(), "epoch", metaHdr.GetEpoch(), "remaining epoch changes", mp.nrEpochsChanges, "rounds modulus", mp.roundsModulus)
}
