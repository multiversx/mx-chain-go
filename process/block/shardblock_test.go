...
func TestShardProcessor_CreateTxBlockBodyWithDirtyAccStateShouldErr(t *testing.T) {
	...
	bl, err := sp.CreateBlockBody(&mock.BlockChainMock{}, &block.Header{PrevRandSeed: []byte("randSeed")}, func() bool { return true })
	// nil block
	assert.Nil(t, bl)
	// error
	assert.Equal(t, process.ErrAccountStateDirty, err)
...
func TestShardProcessor_CreateTxBlockBodyWithNoTimeShouldEmptyBlock(t *testing.T) {
	...
	bl, err := sp.CreateBlockBody(&mock.BlockChainMock{}, &block.Header{PrevRandSeed: []byte("randSeed")}, haveTimeTrue)
	// no error
	assert.Equal(t, process.ErrTimeIsOut, err)
	// no miniblocks
	assert.Nil(t, bl)
...
func TestShardProcessor_CreateTxBlockBodyOK(t *testing.T) {
	...
	sp, _ := blproc.NewShardProcessor(arguments)
	blk, err := sp.CreateBlockBody(&mock.BlockChainMock{}, &block.Header{PrevRandSeed: []byte("randSeed")}, haveTimeTrue)
	assert.NotNil(t, blk)
	assert.Nil(t, err)
}
...
func TestShardProcessor_CreateAndProcessCrossMiniBlocksDstMe(t *testing.T) {
	...
	arguments.DataPool = tdp
	sp, _ := blproc.NewShardProcessor(arguments)
	miniBlockSlice, usedMetaHdrsHashes, noOfTxs, err := sp.CreateAndProcessCrossMiniBlocksDstMe(2, haveTimeTrue)
...
func TestShardProcessor_CreateAndProcessCrossMiniBlocksDstMeProcessPartOfMiniBlocksInMetaBlock(t *testing.T) {
	...
	miniBlocksReturned, usedMetaHdrsHashes, nrTxAdded, err := sp.CreateAndProcessCrossMiniBlocksDstMe(2, haveTimeTrue)
...
func TestShardProcessor_RemoveAndSaveLastNotarizedMetaHdrNoDstMB(t *testing.T) {
	...
	arguments.BlockTracker.AddCrossNotarizedHeader(sharding.MetachainShardId, lastHdr, nil)
