...
func (txs *transactions) createAndProcessMiniBlocksFromMeV2(
	...
	for index := range sortedTxs {
		...
		txs.postProcessorTxsHandler.InitProcessedResults()
...
func (txs *transactions) applyGeneratedSCRs(mbInfo *createAndProcessMiniBlocksInfo) {
	if !txs.flagMixedTxsInMiniBlocks.IsSet() {
		return
	}

	mapAllIntermediateTxs := txs.postProcessorTxsHandler.GetProcessedResults()
...
func (txs *transactions) handleErrorIfNeeded(hei *handleErrorInfo) error {
	...
	errCompute := txs.computeBlockSizeAndGasConsumedBySCRsFromTx(mbInfo, txMbInfo, hei.isMaxBlockSizeReached)
	if errCompute != nil {
		shouldRevertGasAndAccountState = true
		return errCompute
	}
	...
	if hei.senderShardID == hei.receiverShardID {
		...
		gasPenalized := txs.gasHandler.GasPenalized(hei.txHash)
		...
		shouldDoTheSubtraction := gasToBeSubtracted <= mbInfo.gasInfo.GasConsumedByMiniBlocksInSenderShard &&
			...
			gasToBeSubtracted <= mbInfo.mapGasConsumedByMiniBlockInReceiverShard[hei.receiverShardID]
		if shouldDoTheSubtraction {
			...
			mbInfo.mapGasConsumedByMiniBlockInReceiverShard[hei.receiverShardID] -= gasToBeSubtracted
		}
	}
...
func (txs *transactions) revertGasAndAccountsState(hei *handleErrorInfo) {
	...
	txs.gasHandler.RemoveGasPenalized([][]byte{hei.txHash})
...
func (txs *transactions) computeBlockSizeAndGasConsumedBySCRsFromTx(
	...
	mapAllIntermediateTxsInfo := txs.postProcessorTxsHandler.GetProcessedResults()
