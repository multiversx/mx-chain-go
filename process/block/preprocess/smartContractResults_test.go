...
func TestScrsPreprocessor_ProcessBlockTransactionsShouldErrMaxGasLimitPerBlockInSelfShardIsReached(t *testing.T) {
	...
	err := scrPreproc.ProcessBlockTransactions(&block.Header{}, body, haveTimeTrue, false, &process.GasConsumedInfo{})
	assert.Nil(t, err)
...
func TestScrsPreprocessor_RestoreBlockDataIntoPools(t *testing.T) {
	...
	scrRestored, err := scr.RestoreTxsIntoPools(body)
	assert.Equal(t, scrRestored, 1)
	assert.Nil(t, err)

	err = scr.RestoreMiniBlocksIntoPools(body, miniblockPool)
	assert.Nil(t, err)
}
