...
func TestScrsPreProcessor_GetTransactionFromPool(t *testing.T) {
	...
	tdp := initDataPool()
	tdp.UnsignedTransactionsTxPool.AddData([]byte("tx1_hash"), &smartContractResult.SmartContractResult{Nonce: 10}, "1")
	...
	tx, err := process.GetTransactionHandlerFromPool(1, 1, txHash, tdp.UnsignedTransactions())
	require.Nil(t, err)
	require.NotNil(t, tx)
	require.Equal(t, uint64(10), tx.(*smartContractResult.SmartContractResult).Nonce)
}
...
func TestScrsPreprocessor_ProcessMiniBlock(t *testing.T) {
	...
	tdp := initDataPool()
	tdp.UnsignedTransactionsTxPool.AddData([]byte("tx1_hash"), &smartContractResult.SmartContractResult{Nonce: 10}, "0")
