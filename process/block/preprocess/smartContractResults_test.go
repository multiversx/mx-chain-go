...
func TestScrsPreprocessor_ProcessBlockTransactionsShouldWork(t *testing.T) {
	...
	err := scrPreproc.ProcessBlockTransactions(&block.Header{MiniBlockHeaders: []block.MiniBlockHeader{{Hash: miniblockHash}}}, body, nil, haveTimeTrue)
...
func TestScrsPreprocessor_ProcessBlockTransactionsShouldErrMaxGasLimitPerBlockInSelfShardIsReached(t *testing.T) {
	...
	err := scrPreproc.ProcessBlockTransactions(&block.Header{MiniBlockHeaders: []block.MiniBlockHeader{{Hash: miniblockHash, TxCount: 1}}}, body, nil, haveTimeTrue)
	assert.Nil(t, err)
...
func TestScrsPreprocessor_ProcessMiniBlock(t *testing.T) {
	...
	_, _, _, err := scr.ProcessMiniBlock(&miniblock, haveTimeTrue, haveAdditionalTimeFalse, false, false, -1, postProcessorInfoHandlerMock)
