...
func (inPeerBlkBdy *InterceptedPeerBlockBody) IntegrityAndValidity(coordinator sharding.Sharder) error {
...
func (inPeerBlkBdy *InterceptedPeerBlockBody) Integrity(coordinator sharding.Sharder) error {
	if coordinator == nil {
	...
	for _, change := range inPeerBlkBdy.PeerBlockBody {
		if change.ShardIdDest >= coordinator.CurrentNumberOfShards() {
