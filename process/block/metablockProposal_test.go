...
func TestMetaProcessor_selectIncomingMiniBlocks(t *testing.T) {
	...
	t.Run("pending mini blocks returned -> break without adding header", func(t *testing.T) {
		...
		arguments.TxCoordinator = &testscommon.TransactionCoordinatorMock{
			CreateMbsCrossShardDstMeCalled: func(header data.HeaderHandler, processedMiniBlocksInfo map[string]*processedMb.ProcessedMiniBlockInfo) (*process.CreateMbsCrossShardResult, error) {
				return &process.CreateMbsCrossShardResult{PendingMiniBlocks: []block.MiniblockAndHash{{}}, AllDataAvailable: true}, nil
			},
		}
		mp, err := blproc.NewMetaProcessor(arguments)
		require.Nil(t, err)
	...
	t.Run("success: miniblocks added and header referenced", func(t *testing.T) {
		...
		arguments.TxCoordinator = &testscommon.TransactionCoordinatorMock{
			CreateMbsCrossShardDstMeCalled: func(header data.HeaderHandler, processedMiniBlocksInfo map[string]*processedMb.ProcessedMiniBlockInfo) (*process.CreateMbsCrossShardResult, error) {
				return &process.CreateMbsCrossShardResult{AddedMiniBlocks: []block.MiniblockAndHash{{}}, NumTransactions: 3, AllMiniBlocksAdded: true, AllDataAvailable: true}, nil
			},
		}
		mp, err := blproc.NewMetaProcessor(arguments)
		require.Nil(t, err)
