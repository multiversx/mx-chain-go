...
func (inTxBlkBdy *InterceptedTxBlockBody) IntegrityAndValidity(coordinator sharding.Sharder) error {
...
func (inTxBlkBdy *InterceptedTxBlockBody) Integrity(coordinator sharding.Sharder) error {
	if coordinator == nil {
	...
	for _, miniBlock := range inTxBlkBdy.TxBlockBody {
		...
		if miniBlock.ShardID >= coordinator.CurrentNumberOfShards() {
