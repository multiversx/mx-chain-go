...
func (csm *ChainStorerMock) Has(unitType dataRetriever.UnitType, key []byte, priority common.StorageAccessType) error {
	if csm.HasCalled != nil {
		return csm.HasCalled(unitType, key, priority)
	}
...
func (csm *ChainStorerMock) Get(unitType dataRetriever.UnitType, key []byte, priority common.StorageAccessType) ([]byte, error) {
	if csm.GetCalled != nil {
		return csm.GetCalled(unitType, key, priority)
	}
	return nil, nil
}
...
// or if the storage unit underlying implementation reports an error
func (csm *ChainStorerMock) Put(unitType dataRetriever.UnitType, key []byte, value []byte, priority common.StorageAccessType) error {
	if csm.PutCalled != nil {
		return csm.PutCalled(unitType, key, value, priority)
	}
	return nil
}
...
func (csm *ChainStorerMock) GetAll(unitType dataRetriever.UnitType, keys [][]byte, priority common.StorageAccessType) (map[string][]byte, error) {
	if csm.GetAllCalled != nil {
		return csm.GetAllCalled(unitType, keys, priority)
	}
	return nil, nil
}
