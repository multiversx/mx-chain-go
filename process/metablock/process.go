...
type blockProcessor struct {
	...
	OnRequestMiniblock           func(shardID uint32, txHash []byte)
	requestedHeadersHashes       map[string]bool
	mutRequestedHeadersHahses    sync.RWMutex
	requestedMiniblocksHashes    map[string]bool
	mutRequestedMiniblocksHahses sync.RWMutex
	ChRcvAllMiniblocks           chan bool
...
func NewBlockProcessor(
	...
	if requestMiniblockHandler == nil {
		return nil, process.ErrNilRequestMiniblockHeaderHandler
	}
	...
	bp.ChRcvAllMiniblocks = make(chan bool)
...
func (bp *blockProcessor) ProcessBlock(chainHandler data.ChainHandler, headerHandler data.HeaderHandler, bodyHandler data.BodyHandler, haveTime func() time.Duration) error {
	...
	//blockBody, ok := bodyHandler.(*block.MetaBlockBody)
	//if !ok {
	...
	requestedBlockHeaders := bp.requestBlockHeaders(blockHeader)
	...
	if requestedBlockHeaders > 0 {
		...
		err = bp.waitForBlockHeaders(haveTime())
		log.Info(fmt.Sprintf("received %d missing block headers\n", requestedBlockHeaders-len(bp.requestedHeadersHashes)))
		if err != nil {
			return err
		}
	}

	requestedMiniblocks := bp.requestMiniblocks(blockHeader)

	if haveTime() < 0 {
		return process.ErrTimeIsOut
	}

	if requestedMiniblocks > 0 {
		log.Info(fmt.Sprintf("requested %d missing mini block\n", requestedMiniblocks))
		err = bp.waitForMiniblocks(haveTime())
		log.Info(fmt.Sprintf("received %d missing mini block\n", requestedMiniblocks-len(bp.requestedMiniblocksHashes)))
		if err != nil {
			return err
		}
	}
...
func (bp *blockProcessor) CreateGenesisBlock(balances map[string]*big.Int) (rootHash []byte, err error) {
	return []byte("metachain genesis block rooth hash"), nil
}
...
func (bp *blockProcessor) validateHeader(blockChain *blockchain.MetaChain, header *block.MetaBlock) error {
	...
	if blockChain.GetCurrentBlockHeader() == nil {
		...
		if !bytes.Equal(header.PreviousHash, blockChain.GetCurrentBlockHeaderHash()) {
...
func (bp *blockProcessor) isFirstBlockInEpoch(header *block.MetaBlock) bool {
...
func (bp *blockProcessor) receivedHeader(headerHash []byte) {
	bp.mutRequestedHeadersHahses.Lock()
	if len(bp.requestedHeadersHashes) > 0 {
		if bp.requestedHeadersHashes[string(headerHash)] {
			delete(bp.requestedHeadersHashes, string(headerHash))
		}
		lenReqHeadersHashes := len(bp.requestedHeadersHashes)
		bp.mutRequestedHeadersHahses.Unlock()

		if lenReqHeadersHashes == 0 {
			bp.ChRcvAllHeaders <- true
		}
		return
	}
	bp.mutRequestedHeadersHahses.Unlock()
...
// receivedMiniblock is a call back function which is called when a new miniblock
// is added in the miniblocks pool
func (bp *blockProcessor) receivedMiniblock(miniblockHash []byte) {
	bp.mutRequestedMiniblocksHahses.Lock()
	if len(bp.requestedMiniblocksHashes) > 0 {
		if bp.requestedMiniblocksHashes[string(miniblockHash)] {
			delete(bp.requestedMiniblocksHashes, string(miniblockHash))
		}
		lenReqMiniblocksHashes := len(bp.requestedMiniblocksHashes)
		bp.mutRequestedMiniblocksHahses.Unlock()

		if lenReqMiniblocksHashes == 0 {
			bp.ChRcvAllMiniblocks <- true
		}
		return
	}
	bp.mutRequestedMiniblocksHahses.Unlock()
...
//		if miniVal == nil {
//			go bp.OnRequestHeader(senderShardId, []byte(k))
//		}
//	}
//}
...
//		tx := bp.getTransactionFromPool(miniBlock.SenderShardID, miniBlock.ReceiverShardID, txHash)
//		if tx == nil {
//			go bp.OnRequestMiniblock(miniBlock.SenderShardID, txHash)
//		}
//	}
//}

func (bp *blockProcessor) requestBlockHeaders(metablock *block.MetaBlock) int {
	bp.mutRequestedHeadersHahses.Lock()
	requestedHeaders := 0
	missingHeaderHashes := bp.computeMissingHeaders(metablock)
	bp.requestedHeadersHashes = make(map[string]bool)
	if bp.OnRequestHeader != nil {
		for shardId, headerHash := range missingHeaderHashes {
			requestedHeaders++
			bp.requestedHeadersHashes[string(headerHash)] = true
			go bp.OnRequestHeader(shardId, headerHash)
		}
	}
	bp.mutRequestedHeadersHahses.Unlock()
...
func (bp *blockProcessor) computeMissingHeaders(metablock *block.MetaBlock) map[uint32][]byte {
	missingHeaders := make(map[uint32][]byte)
	for i := 0; i < len(metablock.ShardInfo); i++ {
		shardData := metablock.ShardInfo[i]
		header := bp.getHeaderFromPool(shardData.ShardId, shardData.HeaderHash)
		if header == nil {
...
func (bp *blockProcessor) getHeaderFromPool(shardID uint32, headerHash []byte) *block.Header {
	headerPool := bp.dataPool.Headers()
	if headerPool == nil {
...
func (bp *blockProcessor) requestMiniblocks(metablock *block.MetaBlock) int {
	bp.mutRequestedMiniblocksHahses.Lock()
	requestedMiniblocks := 0
	missingMiniblocksHashes := bp.computeMissingMiniblocks(metablock)
	bp.requestedMiniblocksHashes = make(map[string]bool)
	if bp.OnRequestMiniblock != nil {
		for shardId, miniblocksHashes := range missingMiniblocksHashes {
			for _, miniblockHash := range miniblocksHashes {
				requestedMiniblocks++
				bp.requestedMiniblocksHashes[string(miniblockHash)] = true
				go bp.OnRequestMiniblock(shardId, miniblockHash)
			}
		}
	}
	bp.mutRequestedMiniblocksHahses.Unlock()
	return requestedMiniblocks
}

func (bp *blockProcessor) computeMissingMiniblocks(metablock *block.MetaBlock) map[uint32][][]byte {
	missingMiniblocks := make(map[uint32][][]byte)

	for i := 0; i < len(metablock.ShardInfo); i++ {
		shardData := metablock.ShardInfo[i]
		currentShardMissingMiniblocks := make([][]byte, 0)

		for j := 0; j < len(shardData.ShardMiniBlockHeaders); j++ {
			miniblockHash := shardData.ShardMiniBlockHeaders[j].Hash
			miniblock := bp.getMiniblockFromPool(shardData.ShardId, miniblockHash)

			if miniblock == nil {
				currentShardMissingMiniblocks = append(currentShardMissingMiniblocks, miniblockHash)
			}
		}

		if len(currentShardMissingMiniblocks) > 0 {
			missingMiniblocks[shardData.ShardId] = currentShardMissingMiniblocks
		}
	}

	return missingMiniblocks
}

// getShardMiniblockHeaderFromPool gets the miniblock header from a given shard id and a given miniblock header hash
func (bp *blockProcessor) getMiniblockFromPool(shardID uint32, miniblockHash []byte) *block.MiniBlock {
	miniblockPool := bp.dataPool.MiniBlocks()
	if miniblockPool == nil {
		log.Error(process.ErrNilMiniBlockPool.Error())
		return nil
	}

	val, ok := miniblockPool.Peek(miniblockHash)
	if !ok {
		return nil
	}

	v := val.(*block.MiniBlock)
	return v
}
...
func (bp *blockProcessor) waitForBlockHeaders(waitTime time.Duration) error {
	select {
	case <-bp.ChRcvAllHeaders:
		return nil
...
func (bp *blockProcessor) waitForMiniblocks(waitTime time.Duration) error {
	select {
	case <-bp.ChRcvAllMiniblocks:
		return nil
	case <-time.After(waitTime):
		return process.ErrTimeIsOut
	}
