...
func (tlp *txLogProcessor) GetLogFromCache(txHash []byte) (data.LogDataHandler, bool) {
	tlp.mut.RLock()
	defer tlp.mut.RUnlock()
	...
	txLogHandler, err := tlp.GetLog(txHash)
	if err != nil {
		return nil, false
	}

	txLog, ok := txLogHandler.(*transaction.Log)
	if !ok {
		return nil, false
	}
...
func (tlp *txLogProcessor) saveLogToCache(txHash []byte, log *transaction.Log) {
	tlp.mut.Lock()
	tlp.logs = append(tlp.logs, &transaction.LogData{
		TxHash: string(txHash),
		Log:    log,
	})
