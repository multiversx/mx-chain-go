...
func (tlp *txLogProcessor) GetLogFromCache(txHash []byte) (indexer.LogData, bool) {
	tlp.mut.RLock()
	defer tlp.mut.RUnlock()
...
func (tlp *txLogProcessor) GetAllCurrentLogs() []indexer.LogData {
	tlp.mut.RLock()
	logsSlice := make([]indexer.LogData, 0, len(tlp.logs))
	for _, value := range tlp.logs {
		logsSlice = append(logsSlice, value)
	}
	tlp.mut.RUnlock()
...
func (tlp *txLogProcessor) Clean() {
	...
	tlp.logs = make([]indexer.LogData, 0)
	tlp.mut.Unlock()
}
...
func (tlp *txLogProcessor) saveLogToCache(txHash []byte, log *transaction.Log) {
	tlp.mut.Lock()
	tlp.logs = append(tlp.logs, indexer.LogData{
		TxHash:     string(txHash),
		LogHandler: log,
	})
