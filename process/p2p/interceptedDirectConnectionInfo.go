...
type interceptedDirectConnectionInfo struct {
	directConnectionInfo message.DirectConnectionInfo
	numOfShards          uint32
}
...
func createDirectConnectionInfo(marshaller marshal.Marshalizer, buff []byte) (*message.DirectConnectionInfo, error) {
	directConnectionInfo := &message.DirectConnectionInfo{}
...
func (idci *interceptedDirectConnectionInfo) CheckValidity() error {
	shardId, err := strconv.ParseInt(idci.directConnectionInfo.ShardId, 10, 32)
	if err != nil {
		return err
	}
