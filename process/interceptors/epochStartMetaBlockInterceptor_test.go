...
func TestEpochStartMetaBlockInterceptor_ProcessReceivedMessageUnmarshalError(t *testing.T) {
	...
	_, err := esmbi.ProcessReceivedMessage(message, "", &p2pmocks.MessengerStub{})
	require.Error(t, err)
}
...
func TestEpochStartMetaBlockInterceptor_EntireFlowShouldWorkAndSetTheEpoch(t *testing.T) {
	...
	_, err := esmbi.ProcessReceivedMessage(&p2pmocks.P2PMessageMock{DataField: metaBlockBytes}, "peer0", &p2pmocks.MessengerStub{})
	require.NoError(t, err)
