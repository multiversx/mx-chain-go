...
func TestEpochActivation(t *testing.T) {
	t.Parallel()
	ea, err := epochActivation.NewEpochActivation(*createMockConfig())
	require.Nil(t, err)
	require.False(t, ea.IsInterfaceNil())

	ea.EpochConfirmed(200, 0)
	require.False(t, ea.IsESDTEnabled())
}
...
func createMockConfig() *config.EnableEpochs {
	cfg := &config.EnableEpochs{
		...
		MaxNodesChangeEnableEpoch:                         make([]config.MaxNodesChangeConfig, 0),
