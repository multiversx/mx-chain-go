...
func TestHistoryRepositoryFactory_CreateMissingStorersReturnsError(t *testing.T) {
	...
	t.Run("missing ResultsHashesByTxHashUnit", testWithMissingStorer(dataRetriever.MiniBlockUnit))
	...
	t.Run("missing ResultsHashesByTxHashUnit", testWithMissingStorer(dataRetriever.ExecutionResultsUnit))
}
