...
func TestHeaderResolverBase_ProcessReceivedMessageNilValueShouldErr(t *testing.T) {
	...
	rd, err := hdrResBase.ParseReceivedMessage(createRequestMsg(dataRetriever.NonceType, nil))
...
func TestHeaderResolverBase_ProcessReceivedMessageOkValsShouldWork(t *testing.T) {
	...
	rd, err := hdrResBase.ParseReceivedMessage(createRequestMsg(dataRetriever.HashType, expectedValue))
	expectedRequestData := &dataRetriever.RequestData{
