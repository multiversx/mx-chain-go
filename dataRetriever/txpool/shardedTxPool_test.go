...
func Test_NewShardedTxPool(t *testing.T) {
	pool, err := NewShardedTxPool(storageUnit.CacheConfig{})
...
func Test_NewShardedTxPool_ComputesEvictionConfig(t *testing.T) {
	poolAsInterface, err := NewShardedTxPool(storageUnit.CacheConfig{Size: 75000})
	require.Nil(t, err)
...
func Test_AddData_NoPanic_IfNotATransaction(t *testing.T) {
	poolAsInterface, _ := NewShardedTxPool(storageUnit.CacheConfig{})
...
func Test_AddData_CallsOnAddedHandlers(t *testing.T) {
	poolAsInterface, _ := NewShardedTxPool(storageUnit.CacheConfig{Size: 75000})
	pool := poolAsInterface.(*shardedTxPool)
