...
// NewRequesterRequestHandler creates a requestHandler interface implementation with request functions
func NewRequesterRequestHandler(
...
func (rrh *resolverRequestHandler) RequestTrieNodes(destShardID uint32, hashes [][]byte, topic string) {
	...
	if err != nil {
		log.Error("requestByHash.Requester",
			"error", err.Error(),
			"topic", topic,
			"shard", destShardID,
		)
...
func (rrh *resolverRequestHandler) getShardHeaderRequester(shardID uint32) (NonceRequester, error) {
	...
	requester, err := rrh.requestersFinder.CrossShardRequester(factory.ShardBlocksTopic, crossShardID)
	if err != nil {
	...
	if !ok {
		err = fmt.Errorf("%w, topic: %s, current shard ID: %d, cross shard ID: %d, expected NonceRequester",
...
func (rrh *resolverRequestHandler) getMetaHeaderRequester() (NonceRequester, error) {
	...
	headerRequester, ok := requester.(NonceRequester)
	if !ok {
		err = fmt.Errorf("%w, topic: %s, current shard ID: %d, expected NonceRequester",
