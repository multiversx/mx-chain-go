...
func createMockDataPoolArgs() dataPool.DataPoolArgs {
	return dataPool.DataPoolArgs{
		...
		RelayedInnerTransactions:  testscommon.NewShardedDataStub(),
...
func TestNewDataPool_NilRelayedInnerTransactionsShouldErr(t *testing.T) {
	t.Parallel()

	args := createMockDataPoolArgs()
	args.RelayedInnerTransactions = nil
	tdp, err := dataPool.NewDataPool(args)

	assert.Equal(t, dataRetriever.ErrNilRelayedInnerTxDataPool, err)
	assert.Nil(t, tdp)
}
...
func TestNewDataPool_OkValsShouldWork(t *testing.T) {
	...
	assert.True(t, args.RelayedInnerTransactions == tdp.RelayedInnerTransactions())
