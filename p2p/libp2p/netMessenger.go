...
func createMessenger(
	...
	ds, err := NewDirectSender(ctx, h, netMes.directMessageHandler)
	if err != nil {
		return nil, err
	}
	netMes.ds = ds
...
func (netMes *networkMessenger) TrimConnections() error {
	if netMes.connManager == nil {
		return p2p.ErrNilConnManager
	}

	netMes.connManager.TrimOpenConns(netMes.ctx)
	return nil
}
...
func (netMes *networkMessenger) SetTopicValidator(topic string, handler func(message p2p.MessageP2P) error) error {
	netMes.mutTopics.Lock()
...
func (netMes *networkMessenger) SendDirectToConnectedPeer(topic string, buff []byte, peerID p2p.PeerID) error {
