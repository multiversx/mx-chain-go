...
// ID - gets the unique identifier of the go goroutine
func (grd *goRoutineData) ID() string {
	return grd.id
}

// FirstOccurrence - gets the creation date of the go goroutine
func (grd *goRoutineData) FirstOccurrence() time.Time {
	return grd.firstOccurrence
}

// StackTrace - gets the call stack of the go goroutine
