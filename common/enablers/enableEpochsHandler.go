...
func (handler *enableEpochsHandler) createAllFlagsMap() {
	handler.allFlagsDefined = map[core.EnableEpochFlag]flagHandler{
		...
		common.HashForInnerTransactionFlag: {
			isActiveInEpoch: func(epoch uint32) bool {
				return epoch >= handler.enableEpochsConfig.HashForInnerTransactionEnableEpoch
			},
			activationEpoch: handler.enableEpochsConfig.HashForInnerTransactionEnableEpoch,
		},
	}
}
