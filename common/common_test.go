...
func TestGetAccumulatedFeesInEpoch(t *testing.T) {
	t.Parallel()

	t.Run("nil header should return 0", func(t *testing.T) {
		t.Parallel()

		result := common.GetAccumulatedFeesInEpoch(nil)
		require.Equal(t, big.NewInt(0), result)
	})

	t.Run("non-v3 header should return 0", func(t *testing.T) {
		t.Parallel()
		...
		result := common.GetAccumulatedFeesInEpoch(header)
		require.Equal(t, big.NewInt(0), result)
	})

	t.Run("non-v3 meta header should return accumulated fees in epoch", func(t *testing.T) {
		t.Parallel()

		expectedFees := big.NewInt(3000)
		header := &block.MetaBlock{
			Nonce:                  1,
			AccumulatedFeesInEpoch: expectedFees,
		}

		result := common.GetAccumulatedFeesInEpoch(header)
		require.Equal(t, expectedFees, result)
	})

	t.Run("v3 meta header should return fees from last execution result", func(t *testing.T) {
		t.Parallel()

		expectedFees := big.NewInt(4000)
		header := &block.MetaBlockV3{
			LastExecutionResult: &block.MetaExecutionResultInfo{
	...
	t.Run("v3 header with invalid cast should return 0", func(t *testing.T) {
		t.Parallel()
...
func TestGetDeveloperFeesInEpoch(t *testing.T) {
	t.Parallel()

	t.Run("nil header should return 0", func(t *testing.T) {
		t.Parallel()

		result := common.GetDeveloperFeesInEpoch(nil)
		require.Equal(t, big.NewInt(0), result)
	})
	...
	t.Run("non-v3 header should return 0", func(t *testing.T) {
		...
		result := common.GetDeveloperFeesInEpoch(header)
		require.Equal(t, big.NewInt(0), result)
	})

	t.Run("non-v3 meta header should return developer fees in epoch", func(t *testing.T) {
		t.Parallel()

		expectedFees := big.NewInt(2500)
		header := &block.MetaBlock{
			Nonce:          1,
			DevFeesInEpoch: expectedFees,
		}
