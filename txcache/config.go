...
func (config *ConfigSourceMe) verify() error {
	if len(config.Name) == 0 {
		return fmt.Errorf("%w: config.Name is invalid", elrond_go_storage.ErrInvalidConfig)
	}
	if config.NumChunks < numChunksLowerBound || config.NumChunks > numChunksUpperBound {
		return fmt.Errorf("%w: config.NumChunks is invalid", elrond_go_storage.ErrInvalidConfig)
	}
	if config.NumBytesPerSenderThreshold < maxNumBytesPerSenderLowerBound || config.NumBytesPerSenderThreshold > maxNumBytesPerSenderUpperBound {
		return fmt.Errorf("%w: config.NumBytesPerSenderThreshold is invalid", elrond_go_storage.ErrInvalidConfig)
	}
	if config.CountPerSenderThreshold < maxNumItemsPerSenderLowerBound {
		return fmt.Errorf("%w: config.CountPerSenderThreshold is invalid", elrond_go_storage.ErrInvalidConfig)
	}
	if config.EvictionEnabled {
		if config.NumBytesThreshold < maxNumBytesLowerBound || config.NumBytesThreshold > maxNumBytesUpperBound {
			return fmt.Errorf("%w: config.NumBytesThreshold is invalid", elrond_go_storage.ErrInvalidConfig)
		}
		if config.CountThreshold < maxNumItemsLowerBound {
			return fmt.Errorf("%w: config.CountThreshold is invalid", elrond_go_storage.ErrInvalidConfig)
		}
		if config.NumSendersToPreemptivelyEvict < numSendersToPreemptivelyEvictLowerBound {
			return fmt.Errorf("%w: config.NumSendersToPreemptivelyEvict is invalid", elrond_go_storage.ErrInvalidConfig)
		}
	}
...
func (config *ConfigDestinationMe) verify() error {
	...
	if config.MaxNumItems < maxNumItemsLowerBound {
		return fmt.Errorf("%w: config.MaxNumItems is invalid", elrond_go_storage.ErrInvalidConfig)
	}
	if config.MaxNumBytes < maxNumBytesLowerBound || config.MaxNumBytes > maxNumBytesUpperBound {
		return fmt.Errorf("%w: config.MaxNumBytes is invalid", elrond_go_storage.ErrInvalidConfig)
	}
	if config.NumItemsToPreemptivelyEvict < numTxsToPreemptivelyEvictLowerBound {
		return fmt.Errorf("%w: config.NumItemsToPreemptivelyEvict is invalid", elrond_go_storage.ErrInvalidConfig)
	}
