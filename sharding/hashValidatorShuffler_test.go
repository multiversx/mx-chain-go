...
func generateValidatorList(number int) []nodesCoordinator.Validator {
...
func contains(a []nodesCoordinator.Validator, b []nodesCoordinator.Validator) bool {
	var found bool
	for _, va := range a {
		found = false
		for _, vb := range b {
...
func testRemoveValidators(
	t *testing.T,
	initialValidators []nodesCoordinator.Validator,
	validatorsToRemove []nodesCoordinator.Validator,
	remaining []nodesCoordinator.Validator,
	removed []nodesCoordinator.Validator,
	maxToRemove int,
) {
...
func testDistributeValidators(
	t *testing.T,
	initialMap map[uint32][]nodesCoordinator.Validator,
	resultedMap map[uint32][]nodesCoordinator.Validator,
	distributedNodes []nodesCoordinator.Validator,
) {
	totalResultingValidators := make([]nodesCoordinator.Validator, 0)
	totalLen := 0
	...
	totalValidators := make([]nodesCoordinator.Validator, 0)
...
func numberMatchingNodes(searchList []nodesCoordinator.Validator, toFind []nodesCoordinator.Validator) int {
	nbFound := 0
...
func testLeaving(
	...
	prevLeaving []nodesCoordinator.Validator,
	newLeaving []nodesCoordinator.Validator,
) (int, map[uint32]int) {
...
func testShuffledOut(
	...
	newEligible map[uint32][]nodesCoordinator.Validator,
	shuffledOut []nodesCoordinator.Validator,
	prevleaving []nodesCoordinator.Validator,
	newleaving []nodesCoordinator.Validator,
) {
...
func getValidatorsInMap(valMap map[uint32][]nodesCoordinator.Validator) []nodesCoordinator.Validator {
	result := make([]nodesCoordinator.Validator, 0)
...
func Test_promoteWaitingToEligible_2xMoreWaitingThanRemainingSize(t *testing.T) {
	...
	for k := range eligibleMap {
		eligibleMap[k] = make([]nodesCoordinator.Validator, 0)
	}
	err = moveMaxNumNodesToMap(eligibleMap, waitingMap, numMeta, numShard)
	assert.Nil(t, err)
...
func Test_removeValidatorFromListFirst(t *testing.T) {
	...
	validatorsCopy := make([]nodesCoordinator.Validator, len(validators))
...
func Test_removeValidatorsFromListRemoveFromStart(t *testing.T) {
	...
	validators := generateValidatorList(30)
	validatorsCopy := make([]nodesCoordinator.Validator, len(validators))
	validatorsToRemove := make([]nodesCoordinator.Validator, 0)
...
func Test_removeDupplicates_NoDupplicates(t *testing.T) {
	...
	firstListCopy := make([]nodesCoordinator.Validator, len(firstList))
	copy(firstListCopy, firstList)

	secondListCopy := make([]nodesCoordinator.Validator, len(secondList))
...
func Test_removeDupplicates_AllDupplicates(t *testing.T) {
	...
	firstList := generateValidatorList(30)
	secondList := make([]nodesCoordinator.Validator, len(firstList))
...
func Test_shuffleList(t *testing.T) {
	...
	validators := generateValidatorList(30)
	validatorsCopy := make([]nodesCoordinator.Validator, 0)
...
func Test_distributeValidatorsNilOrEmptyDestination(t *testing.T) {
	...
	err = distributeValidators(make(map[uint32][]nodesCoordinator.Validator), validatorsToDistribute, randomness, false)
	assert.Equal(t, ErrNilOrEmptyDestinationForDistribute, err)
}
...
func Test_shuffleOutNodesNoLeaving(t *testing.T) {
	...
	nbShards := uint32(2)
	var leaving []nodesCoordinator.Validator
	...
	stillRemainingInLeaving := make([]nodesCoordinator.Validator, 0)
	...
	shuffleOutList := make([]nodesCoordinator.Validator, 0)
...
func Test_shuffleOutNodesWithLeaving(t *testing.T) {
	...
	waitingNodesPerShard := 40
	nbShards := uint32(2)
	leaving := make([]nodesCoordinator.Validator, 0)
...
func TestRandHashShuffler_UpdateNodeListsNoReSharding(t *testing.T) {
	...
	leavingNodes := make([]nodesCoordinator.Validator, 0)
	extraLeavingNodes := make([]nodesCoordinator.Validator, 0)
	newNodes := make([]nodesCoordinator.Validator, 0)
...
func testUpdateNodeListsAndCheckWaitingList(t *testing.T, beforeFix bool) {
	...
	initialWaitingListCopy := make(map[uint32][]nodesCoordinator.Validator, len(args.Waiting))
	for shID, val := range args.Waiting {
		initialWaitingListCopy[shID] = make([]nodesCoordinator.Validator, len(val))
...
func TestRandHashShuffler_UpdateNodeListsWithRangeOnMaps(t *testing.T) {
	...
	for _, shuffle := range shuffleBetweenShards {
		...
		leavingValidators := make([]nodesCoordinator.Validator, numLeaving)
...
func TestRandHashShuffler_UpdateNodeListsNoReShardingIntraShardShuffling(t *testing.T) {
	...
	leavingNodes := make([]nodesCoordinator.Validator, 0)
	additionalLeavingNodes := make([]nodesCoordinator.Validator, 0)
	newNodes := make([]nodesCoordinator.Validator, 0)
...
func TestRandHashShuffler_RemoveLeavingNodesFromValidatorMaps_2Eligible2Waiting2NonExisting(t *testing.T) {
	...
	leavingValidators = append(leavingValidators, []nodesCoordinator.Validator{
		eligibleMap[core.MetachainShardId][0],
...
func TestRandHashShuffler_RemoveLeavingNodesFromValidatorMaps_2FromEligible2FromWaitingFrom3Waiting(t *testing.T) {
	...
	leavingValidators := make([]nodesCoordinator.Validator, 0)
	leavingValidators = append(leavingValidators, []nodesCoordinator.Validator{
		eligibleMap[core.MetachainShardId][0],
		eligibleMap[core.MetachainShardId][5]}...)
	leavingValidators = append(leavingValidators, []nodesCoordinator.Validator{
...
func TestRandHashShuffler_UpdateNodeLists_WithUnstakeLeaving(t *testing.T) {
	...
	unStakeLeaving := make(map[uint32][]nodesCoordinator.Validator)
	additionalLeaving := make(map[uint32][]nodesCoordinator.Validator)
	unStakeLeaving[core.MetachainShardId] = []nodesCoordinator.Validator{
		eligibleMap[core.MetachainShardId][0],
	...
	unStakeLeaving[0] = []nodesCoordinator.Validator{
		waitingMap[0][0],
		waitingMap[0][waitingPerShard/2]}

	unStakeLeaving[1] = []nodesCoordinator.Validator{
		eligibleMap[1][0],
		eligibleMap[1][1],
		eligibleMap[1][2],
		eligibleMap[1][eligiblePerShard-1]}

	additionaLeavingList := make([]nodesCoordinator.Validator, 0)
	...
	arg := ArgsUpdateNodes{
		...
		AdditionalLeaving: make([]nodesCoordinator.Validator, 0),
...
func TestRandHashShuffler_UpdateNodeLists_WithUnstakeLeaving_NotEnoughRemaining(t *testing.T) {
	...
	eligibleMap[core.MetachainShardId] = make([]nodesCoordinator.Validator, 0) // eligible - no validators
...
func TestRandHashShuffler_UpdateNodeLists_WithAdditionalLeaving(t *testing.T) {
	...
	additionalLeaving := make(map[uint32][]nodesCoordinator.Validator)
	unstakeLeaving := make(map[uint32][]nodesCoordinator.Validator)
	additionalLeaving[core.MetachainShardId] = []nodesCoordinator.Validator{
		eligibleMap[core.MetachainShardId][0],
		eligibleMap[core.MetachainShardId][eligiblePerShard-1],
		waitingMap[core.MetachainShardId][0],
		waitingMap[core.MetachainShardId][1],
	...
	additionalLeaving[0] = []nodesCoordinator.Validator{
		waitingMap[0][0],
		waitingMap[0][waitingPerShard/2]}

	additionalLeaving[1] = []nodesCoordinator.Validator{
		eligibleMap[1][0],
		eligibleMap[1][1],
		eligibleMap[1][2],
		eligibleMap[1][eligiblePerShard-1]}
...
func TestRandHashShuffler_UpdateNodeLists_All(t *testing.T) {
	...
	removedFromMeta := []nodesCoordinator.Validator{firstRemovedMeta, secondRemovedMeta}
	...
	remainingInMeta := []nodesCoordinator.Validator{notRemovedMeta}
	...
	removedFromShard0 := []nodesCoordinator.Validator{firstRemovedShard0, secondRemovedShard0}
	...
	removedFromShard1 := []nodesCoordinator.Validator{firstRemovedShard1}
...
func TestRandHashShuffler_UpdateNodeLists_WithNewNodes_NilOrEmptyWaiting(t *testing.T) {
	...
	args = ArgsUpdateNodes{
		Eligible:          eligibleMap,
		Waiting:           make(map[uint32][]nodesCoordinator.Validator),
		NewNodes:          newNodes,
		UnStakeLeaving:    leavingNodes,
		AdditionalLeaving: additionalLeaving,
		Rand:              randomness,
...
func TestRandHashShuffler_UpdateNodeLists_WithNewNodes_WithWaiting_WithLeaving(t *testing.T) {
	...
	leavingNodes = append(leavingNodes, []nodesCoordinator.Validator{
		eligibleMap[core.MetachainShardId][0],
		eligibleMap[core.MetachainShardId][1],
		waitingMap[core.MetachainShardId][2],
	}...)

	additionalLeaving = append(additionalLeaving, []nodesCoordinator.Validator{
		eligibleMap[core.MetachainShardId][0],
		waitingMap[core.MetachainShardId][1],
	}...)
...
func prepareListsFromMaps(unstakeLeaving map[uint32][]nodesCoordinator.Validator, additionalLeaving map[uint32][]nodesCoordinator.Validator) ([]nodesCoordinator.Validator, []nodesCoordinator.Validator) {
	return getValidatorsInMap(unstakeLeaving), getValidatorsInMap(additionalLeaving)
}

func verifyResultsIntraShardShuffling(
	t *testing.T,
	eligible []nodesCoordinator.Validator,
	waiting []nodesCoordinator.Validator,
	unstakeLeaving []nodesCoordinator.Validator,
	additionalLeaving []nodesCoordinator.Validator,
	newEligible []nodesCoordinator.Validator,
	newWaiting []nodesCoordinator.Validator,
	leaving []nodesCoordinator.Validator,
	stillRemaining []nodesCoordinator.Validator) {

	removedNodes := make([]nodesCoordinator.Validator, 0)
...
func listToValidatorMap(validators []nodesCoordinator.Validator) map[string]nodesCoordinator.Validator {
	validatorMap := make(map[string]nodesCoordinator.Validator)
