...
func removeDupplicates(unstake []core.Validator, additionalLeaving []core.Validator) []core.Validator {
	additionalCopy := make([]core.Validator, 0, len(additionalLeaving))
...
func removeNodesFromMap(
	existingNodes map[uint32][]core.Validator,
	leavingNodes []core.Validator,
	numToRemove map[uint32]int,
) (map[uint32][]core.Validator, []core.Validator) {
...
func removeNodesFromShard(existingNodes map[uint32][]core.Validator, leavingNodes []core.Validator, shard uint32, nbToRemove int) ([]core.Validator, int) {
...
func createListsForAllShards(shardMap map[uint32][]core.Validator, shards uint32) {
	for shardId := uint32(0); shardId < shards; shardId++ {
		if shardMap[shardId] == nil {
			shardMap[shardId] = make([]core.Validator, 0)
		}
	}

	if shardMap[core.MetachainShardId] == nil {
		shardMap[core.MetachainShardId] = make([]core.Validator, 0)
	}
}
...
func removeLeavingNodesNotExistingInEligibleOrWaiting(
	...
	notFoundValidators := make([]core.Validator, 0)
...
) (map[uint32][]core.Validator, map[uint32][]core.Validator, []core.Validator) {

	stillRemainingInLeaving := make([]core.Validator, len(leaving))
...
func removeLeavingNodes(
	...
	waiting map[uint32][]core.Validator,
	numToRemove map[uint32]int,
	stillRemainingInLeaving []core.Validator,
	minNodesMeta int,
	minNodesPerShard int,
) (map[uint32][]core.Validator, map[uint32][]core.Validator, []core.Validator) {
...
func computeMinNumberOfNodes(eligible map[uint32][]core.Validator, waiting map[uint32][]core.Validator, shardId uint32, minNodesMeta int, minNodesPerShard int) int {
...
func shuffleOutNodes(
	...
	shuffledOutMap := make(map[uint32][]core.Validator)
	newEligible := make(map[uint32][]core.Validator)
	var shardShuffledOut []core.Validator
...
// the hash result.
func shuffleList(validators []core.Validator, randomness []byte) []core.Validator {
	keys := make([]string, len(validators))
	mapValidators := make(map[string]core.Validator)
	var concat []byte
	...
	result := make([]core.Validator, len(validators))
...
func removeValidatorsFromList(
	...
	resultedList := make([]core.Validator, 0)
	resultedList = append(resultedList, validatorList...)
	removed := make([]core.Validator, 0)
...
func removeValidatorFromList(validatorList []core.Validator, index int) []core.Validator {
...
func removeValidatorFromListKeepOrder(validatorList []core.Validator, index int) []core.Validator {
	indexNotOK := index > len(validatorList)-1 || index < 0
	if indexNotOK {
		return validatorList
	}
...
func copyValidatorMap(validatorsMap map[uint32][]core.Validator) map[uint32][]core.Validator {
	result := make(map[uint32][]core.Validator)

	for shardId, validators := range validatorsMap {
		elems := make([]core.Validator, 0)
...
func moveNodesToMap(destination map[uint32][]core.Validator, source map[uint32][]core.Validator) error {
	if destination == nil {
	...
	for shardId, validators := range source {
		...
		source[shardId] = make([]core.Validator, 0)
	}
...
func moveMaxNumNodesToMap(
	destination map[uint32][]core.Validator,
	source map[uint32][]core.Validator,
	numMeta uint32,
	numShard uint32,
) error {
	if destination == nil {
...
func computeNeededNodes(destination []core.Validator, source []core.Validator, maxNumNodes uint32) uint32 {
...
func distributeValidators(destLists map[uint32][]core.Validator, validators []core.Validator, randomness []byte, balanced bool) error {
	if len(destLists) == 0 {
		return ErrNilOrEmptyDestinationForDistribute
	}
...
func equalizeValidatorsLists(destLists map[uint32][]core.Validator, validators []core.Validator) []core.Validator {
...
func getMaxListSize(lists map[uint32][]core.Validator) int {
	var maxSize int
...
func sortKeys(nodes map[uint32][]core.Validator) []uint32 {
