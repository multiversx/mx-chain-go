...
func generateValidatorList(number int) []core.Validator {
	v := make([]core.Validator, number)
...
func contains(a []core.Validator, b []core.Validator) bool {
	var found bool
	for _, va := range a {
		found = false
		for _, vb := range b {
...
func testRemoveValidators(
	...
	validatorsToRemove []core.Validator,
	remaining []core.Validator,
	removed []core.Validator,
	maxToRemove int,
) {
...
func testDistributeValidators(
	t *testing.T,
	initialMap map[uint32][]core.Validator,
	resultedMap map[uint32][]core.Validator,
	distributedNodes []core.Validator,
) {
	totalResultingValidators := make([]core.Validator, 0)
	totalLen := 0
	...
	totalValidators := make([]core.Validator, 0)
...
func numberMatchingNodes(searchList []core.Validator, toFind []core.Validator) int {
	nbFound := 0
...
func testLeaving(
	...
	prevLeaving []core.Validator,
	newLeaving []core.Validator,
) (int, map[uint32]int) {
...
func testShuffledOut(
	...
	newEligible map[uint32][]core.Validator,
	shuffledOut []core.Validator,
	prevleaving []core.Validator,
	newleaving []core.Validator,
) {
...
func getValidatorsInMap(valMap map[uint32][]core.Validator) []core.Validator {
	result := make([]core.Validator, 0)
...
func Test_promoteWaitingToEligible_2xMoreWaitingThanRemainingSize(t *testing.T) {
	...
	for k := range eligibleMap {
		eligibleMap[k] = make([]core.Validator, 0)
	}
	err = moveMaxNumNodesToMap(eligibleMap, waitingMap, numMeta, numShard)
	assert.Nil(t, err)
...
func Test_removeValidatorFromListFirst(t *testing.T) {
	...
	validatorsCopy := make([]core.Validator, len(validators))
...
func Test_removeValidatorsFromListRemoveFromStart(t *testing.T) {
	...
	validators := generateValidatorList(30)
	validatorsCopy := make([]core.Validator, len(validators))
	validatorsToRemove := make([]core.Validator, 0)
...
func Test_removeDupplicates_NoDupplicates(t *testing.T) {
	...
	firstListCopy := make([]core.Validator, len(firstList))
	copy(firstListCopy, firstList)

	secondListCopy := make([]core.Validator, len(secondList))
...
func Test_removeDupplicates_AllDupplicates(t *testing.T) {
	...
	firstList := generateValidatorList(30)
	secondList := make([]core.Validator, len(firstList))
...
func Test_shuffleList(t *testing.T) {
	...
	validators := generateValidatorList(30)
	validatorsCopy := make([]core.Validator, 0)
...
func Test_distributeValidatorsNilOrEmptyDestination(t *testing.T) {
	...
	err = distributeValidators(make(map[uint32][]core.Validator), validatorsToDistribute, randomness, false)
	assert.Equal(t, ErrNilOrEmptyDestinationForDistribute, err)
}
...
func Test_shuffleOutNodesNoLeaving(t *testing.T) {
	...
	stillRemainingInLeaving := make([]core.Validator, 0)
	...
	shuffleOutList := make([]core.Validator, 0)
...
func TestRandHashShuffler_UpdateNodeListsNoReSharding(t *testing.T) {
	...
	leavingNodes := make([]core.Validator, 0)
	extraLeavingNodes := make([]core.Validator, 0)
	newNodes := make([]core.Validator, 0)
...
func TestRandHashShuffler_UpdateNodeListsWithUnstakeLeavingRemovesFromEligible(t *testing.T) {
	...
	args.UnStakeLeaving = []core.Validator{
...
func testUpdateNodeListsAndCheckWaitingList(t *testing.T, beforeFix bool) {
	...
	initialWaitingListCopy := make(map[uint32][]core.Validator, len(args.Waiting))
	for shID, val := range args.Waiting {
		initialWaitingListCopy[shID] = make([]core.Validator, len(val))
...
func TestRandHashShuffler_UpdateNodeListsWithRangeOnMaps(t *testing.T) {
	...
	for _, shuffle := range shuffleBetweenShards {
		...
		allValidators := make([]core.Validator, 0)
		...
		leavingValidators := make([]core.Validator, numLeaving)
...
func TestRandHashShuffler_UpdateNodeListsNoReShardingIntraShardShuffling(t *testing.T) {
	...
	leavingNodes := make([]core.Validator, 0)
	additionalLeavingNodes := make([]core.Validator, 0)
	newNodes := make([]core.Validator, 0)
...
func TestRandHashShuffler_RemoveLeavingNodesNotExistingInEligibleOrWaiting_WithAllKnown(t *testing.T) {
	...
	leavingValidators := []core.Validator{
...
func TestRandHashShuffler_RemoveLeavingNodesFromValidatorMaps_2Eligible2Waiting2NonExisting(t *testing.T) {
	...
	leavingValidators = append(leavingValidators, []core.Validator{
		eligibleMap[core.MetachainShardId][0],
...
func TestRandHashShuffler_RemoveLeavingNodesFromValidatorMaps_2FromEligible2FromWaitingFrom3Waiting(t *testing.T) {
	...
	leavingValidators := make([]core.Validator, 0)
	leavingValidators = append(leavingValidators, []core.Validator{
		eligibleMap[core.MetachainShardId][0],
		eligibleMap[core.MetachainShardId][5]}...)
	leavingValidators = append(leavingValidators, []core.Validator{
...
func TestRandHashShuffler_UpdateNodeLists_WithUnstakeLeaving(t *testing.T) {
	...
	unStakeLeaving := make(map[uint32][]core.Validator)
	additionalLeaving := make(map[uint32][]core.Validator)
	unStakeLeaving[core.MetachainShardId] = []core.Validator{
		eligibleMap[core.MetachainShardId][0],
	...
	unStakeLeaving[0] = []core.Validator{
		waitingMap[0][0],
		waitingMap[0][waitingPerShard/2]}

	unStakeLeaving[1] = []core.Validator{
		eligibleMap[1][0],
		eligibleMap[1][1],
		eligibleMap[1][2],
		eligibleMap[1][eligiblePerShard-1]}

	additionaLeavingList := make([]core.Validator, 0)
	...
	arg := ArgsUpdateNodes{
		...
		AdditionalLeaving: make([]core.Validator, 0),
...
func TestRandHashShuffler_UpdateNodeLists_WithUnstakeLeaving_NotEnoughRemaining(t *testing.T) {
	...
	eligibleMap[core.MetachainShardId] = make([]core.Validator, 0) // eligible - no validators
...
func TestRandHashShuffler_UpdateNodeLists_WithAdditionalLeaving(t *testing.T) {
	...
	additionalLeaving := make(map[uint32][]core.Validator)
	unstakeLeaving := make(map[uint32][]core.Validator)
	additionalLeaving[core.MetachainShardId] = []core.Validator{
		eligibleMap[core.MetachainShardId][0],
		eligibleMap[core.MetachainShardId][eligiblePerShard-1],
		waitingMap[core.MetachainShardId][0],
		waitingMap[core.MetachainShardId][1],
	...
	additionalLeaving[0] = []core.Validator{
		waitingMap[0][0],
		waitingMap[0][waitingPerShard/2]}

	additionalLeaving[1] = []core.Validator{
		eligibleMap[1][0],
		eligibleMap[1][1],
		eligibleMap[1][2],
		eligibleMap[1][eligiblePerShard-1]}
...
func TestRandHashShuffler_UpdateNodeLists_All(t *testing.T) {
	...
	removedFromMeta := []core.Validator{firstRemovedMeta, secondRemovedMeta}
	...
	remainingInMeta := []core.Validator{notRemovedMeta}
	...
	removedFromShard0 := []core.Validator{firstRemovedShard0, secondRemovedShard0}
	...
	removedFromShard1 := []core.Validator{firstRemovedShard1}
...
func TestRandHashShuffler_UpdateNodeLists_WithNewNodes_NilOrEmptyWaiting(t *testing.T) {
	...
	args = ArgsUpdateNodes{
		Eligible:          eligibleMap,
		Waiting:           make(map[uint32][]core.Validator),
		NewNodes:          newNodes,
		UnStakeLeaving:    leavingNodes,
		AdditionalLeaving: additionalLeaving,
		Rand:              randomness,
...
func TestRandHashShuffler_UpdateNodeLists_WithNewNodes_WithWaiting_WithLeaving(t *testing.T) {
	...
	leavingNodes = append(leavingNodes, []core.Validator{
		eligibleMap[core.MetachainShardId][0],
		eligibleMap[core.MetachainShardId][1],
		waitingMap[core.MetachainShardId][2],
	}...)

	additionalLeaving = append(additionalLeaving, []core.Validator{
		eligibleMap[core.MetachainShardId][0],
		waitingMap[core.MetachainShardId][1],
	}...)
...
func prepareListsFromMaps(unstakeLeaving map[uint32][]core.Validator, additionalLeaving map[uint32][]core.Validator) ([]core.Validator, []core.Validator) {
	return getValidatorsInMap(unstakeLeaving), getValidatorsInMap(additionalLeaving)
}

func verifyResultsIntraShardShuffling(
	t *testing.T,
	eligible []core.Validator,
	waiting []core.Validator,
	unstakeLeaving []core.Validator,
	additionalLeaving []core.Validator,
	newEligible []core.Validator,
	newWaiting []core.Validator,
	leaving []core.Validator,
	stillRemaining []core.Validator) {

	removedNodes := make([]core.Validator, 0)
...
func listToValidatorMap(validators []core.Validator) map[string]core.Validator {
	validatorMap := make(map[string]core.Validator)
