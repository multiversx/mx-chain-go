...
type NodesCoordinatorHelper interface {
	ValidatorsWeights(validators []core.Validator) ([]uint32, error)
	ComputeAdditionalLeaving(allValidators []*state.ShardValidatorInfo) (map[uint32][]core.Validator, error)
...
type ValidatorsDistributor interface {
	DistributeValidators(destination map[uint32][]core.Validator, source map[uint32][]core.Validator, rand []byte, balanced bool) error
	IsInterfaceNil() bool
}
