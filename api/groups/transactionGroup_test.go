...
func TestSendTransaction_ErrorWhenFacadeSendTransactionError(t *testing.T) {
	...
	facade := mock.FacadeStub{
		CreateTransactionHandler: func(nonce uint64, value string, receiver string, receiverUsername []byte, sender string, senderUsername []byte, gasPrice uint64, gasLimit uint64, data []byte, signatureHex string, chainID string, version uint32, options uint32, guardian string, guardianSigHex string) (*dataTx.Transaction, []byte, error) {
			return nil, nil, nil
		},
