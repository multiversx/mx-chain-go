...
var _ epochStart.TransactionsSyncHandler = (*transactionsSync)(nil)
...
type transactionsSync struct {
	...
	storage                 map[block.Type]storage.Storer
...
func NewTransactionsSyncer(args ArgsNewTransactionsSyncer) (*transactionsSync, error) {
	...
	var err error
	ts.storage = make(map[block.Type]storage.Storer)
...
func (ts *transactionsSync) receivedTransaction(txHash []byte, val interface{}) {
	...
	if !ok {
		...
		if !ok {
			ts.mutPendingTx.Unlock()
			log.Error("transactionsSync.receivedTransaction", "tx hash", txHash, "error", epochStart.ErrWrongTypeAssertion)
			return
		}
...
func (ts *transactionsSync) receivedValidatorInfo(txHash []byte, val interface{}) {
	...
	if !ok {
		ts.mutPendingTx.Unlock()
		log.Error("transactionsSync.receivedValidatorInfo", "tx hash", txHash, "error", epochStart.ErrWrongTypeAssertion)
		return
	}
...
func (ts *transactionsSync) getTransactionFromPool(txHash []byte) (data.TransactionHandler, bool) {
	...
	tx, ok := val.(data.TransactionHandler)
	if !ok {
		log.Error("transactionsSync.getTransactionFromPool", "tx hash", txHash, "error", epochStart.ErrWrongTypeAssertion)
		return nil, false
	}
...
func (ts *transactionsSync) getValidatorInfoFromPool(txHash []byte) (*state.ShardValidatorInfo, bool) {
	...
	validatorInfo, ok := val.(*state.ShardValidatorInfo)
	if !ok {
		log.Error("transactionsSync.getValidatorInfoFromPool", "tx hash", txHash, "error", epochStart.ErrWrongTypeAssertion)
		return nil, false
	}
...
func (ts *transactionsSync) GetTransactions() (map[string]data.TransactionHandler, error) {
	...
	if !ts.syncedAll {
		return nil, epochStart.ErrNotSynced
	}
