...
type StartOfEpochNodesConfigHandler interface {
	NodesConfigFromMetaBlock(currMetaBlock data.HeaderHandler, prevMetaBlock data.HeaderHandler) (sharding.NodesCoordinatorRegistryHandler, uint32, error)
	IsInterfaceNil() bool
}
...
type StartInEpochNodesCoordinator interface {
	...
	NodesCoordinatorToRegistry() sharding.NodesCoordinatorRegistryHandler
