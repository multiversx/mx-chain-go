...
func (pcf *processComponentsFactory) Create() (*processComponents, error) {
	...
	err = pcf.runTypeComponents.GenesisMetaBlockCreator().SetValidatorRootHashOnGenesisMetaBlock(genesisBlocks[core.MetachainShardId], validatorStatsRootHash)
	if err != nil {
		return nil, err
	}
...
func (pcf *processComponentsFactory) newShardResolverContainerFactory(
	...
	resolversContainerFactory, err := pcf.runTypeComponents.ShardResolversContainerCreator().CreateShardResolversContainerFactory(resolversContainerFactoryArgs)
	if err != nil {
		return nil, err
	}
...
func (pcf *processComponentsFactory) newRequestersContainerFactory(
	...
	if shardCoordinator.SelfId() < shardCoordinator.NumberOfShards() {
		return pcf.runTypeComponents.RequesterContainerCreator().CreateRequesterContainerFactory(requestersContainerFactoryArgs)
	}
	if shardCoordinator.SelfId() == core.MetachainShardId {
...
func (pcf *processComponentsFactory) newShardInterceptorContainerFactory(
	...
	interceptorContainerFactory, err := pcf.runTypeComponents.InterceptorsContainerCreator().CreateInterceptorsContainerFactory(shardInterceptorsContainerFactoryArgs)
	if err != nil {
		return nil, nil, err
	}
...
func checkProcessComponentsArgs(args ProcessComponentsFactoryArgs) error {
	...
	if check.IfNil(args.RunTypeComponents.GenesisMetaBlockCreator()) {
	...
	if check.IfNil(args.RunTypeComponents.InterceptorsContainerCreator()) {
		return fmt.Errorf("%s: %w", baseErrMessage, errorsMx.ErrNilInterceptorsContainerFactoryCreator)
	}
	if check.IfNil(args.RunTypeComponents.RequesterContainerCreator()) {
		return fmt.Errorf("%s: %w", baseErrMessage, errorsMx.ErrNilRequesterContainerFactoryCreator)
	}
	if check.IfNil(args.RunTypeComponents.ShardResolversContainerCreator()) {
