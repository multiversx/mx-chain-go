...
func TestEachSnapshotCreatesOwnDatabase(t *testing.T) {
	...
	for _, testVal := range testVals {
		...
		_ = tr.Commit()
		_ = tr.TakeSnapshot(tr.root.getHash())
		for trieStorage.snapshotsBuffer.len() != 0 {
			time.Sleep(snapshotDelay)
		}
...
func TestTrieCheckpoint(t *testing.T) {
	...
	err = tr.SetCheckpoint(tr.root.getHash())
	assert.Nil(t, err)
...
func TestTrieSnapshottingAndCheckpointConcurrently(t *testing.T) {
	...
	err := tr.TakeSnapshot(tr.root.getHash())
	assert.Nil(t, err)
	for trieStorage.snapshotsBuffer.len() != 0 {
	...
	for i := 0; i < numSnapshots; i++ {
		go func() {
			rootHash, _ := tr.Root()
			assert.Nil(t, tr.TakeSnapshot(rootHash))
			snapshotWg.Done()
		}()
	}
	...
	for i := 0; i < numCheckpoints; i++ {
		go func() {
			rootHash, _ := tr.Root()
			assert.Nil(t, tr.SetCheckpoint(rootHash))
			checkpointWg.Done()
		}()
	}
