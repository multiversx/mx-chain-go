...
// Used for testing
func newEmpty() *Trie {
	trie, _ := NewTrie(make([]byte, 0), nil, getHasher(), getMarsh())
	return trie
}
...
func TestNewTrie(t *testing.T) {
	...
	fmt.Printf("State root hash of an empty trie is: %x\n", trie.Hash())
}
...
func TestNull(t *testing.T) {
	...
	assert.Equal(t, value, trie.Get(key))
}

func TestMissingRoot(t *testing.T) {
	trie, err := NewTrie(getHasher().Compute("AAAA"), nil, getHasher(), getMarsh())
