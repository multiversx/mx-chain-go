...
func TestShardedDataAddData(t *testing.T) {
	t.Parallel()
	txp := shardedData.NewShardedData(nil)

	txp.AddData([]byte("hash_tx1"), &transaction.Transaction{Nonce: 1}, 1)

	has := txp.ShardDataStore(1).Has([]byte("hash_tx1"))
	assert.True(t, has, "Key was not added to minipool")
	assert.True(t, txp.ShardDataStore(1).Len() == 1,
		"Transaction pool length is not 1 after one element was added")

	txp.AddData([]byte("hash_tx2"), &transaction.Transaction{Nonce: 2}, 2)

	assert.False(t, txp.ShardDataStore(1).Has([]byte("hash_tx2")))
	assert.True(t, txp.ShardDataStore(2).Has([]byte("hash_tx2")))
}

func TestShardedDataStorageEvictsTx(t *testing.T) {
	t.Parallel()
	size := 1000
	txp := shardedData.NewShardedData(nil)
	for i := 1; i < size+2; i++ {
	...
	assert.Equal(t, size, txp.ShardDataStore(1).Len(),
		"Transaction pool entries excedes the maximum configured number")
}

func TestShardedDataNoDuplicates(t *testing.T) {
	t.Parallel()
	txp := shardedData.NewShardedData(nil)

	txp.AddData([]byte("tx_hash1"), &transaction.Transaction{Nonce: 1}, 1)
	txp.AddData([]byte("tx_hash1"), &transaction.Transaction{Nonce: 1}, 1)
	assert.Equal(t, 1, txp.ShardDataStore(1).Len(),
		"Transaction pool should not contain duplicates")
}

func TestShardedDataAddDatasInParallel(t *testing.T) {
	t.Parallel()
	txp := shardedData.NewShardedData(nil)
...
func TestShardedDataRemoveData(t *testing.T) {
	t.Parallel()
	txp := shardedData.NewShardedData(nil)
	...
	assert.Equal(t, 0, txp.ShardDataStore(1).Len(),
	...
	assert.Equal(t, 1, txp.ShardDataStore(1).Len(),
		"AddData failed, length should be 1")
	assert.Equal(t, 2, txp.ShardDataStore(2).Len(),
	...
	assert.Equal(t, 1, txp.ShardDataStore(2).Len(),
		"FindAndRemoveData failed, length should be 1 in shard 2")
}

func TestShardedDataClear(t *testing.T) {
	t.Parallel()
	txp := shardedData.NewShardedData(nil)
	txp.Clear()
	txp.ClearMiniPool(1)
	...
	txp.ClearMiniPool(2)
	assert.Equal(t, 0, txp.ShardDataStore(2).Len(),
		"Mini pool for shard 2 should be empty after clear")
	assert.Equal(t, 1, txp.ShardDataStore(1).Len(),
		"Mini pool for shard 1 should still have one element")

	txp.Clear()
	assert.Nil(t, txp.ShardStore(1), "Shard 1 should not be in the store anymore")
	assert.Nil(t, txp.ShardStore(2), "Shard 2 should not be in the store anymore")
}

func TestShardedDataMergeShardStores(t *testing.T) {
	t.Parallel()
	txp := shardedData.NewShardedData(nil)
	txp.AddData([]byte("tx_hash1"), &transaction.Transaction{Nonce: 1}, 1)
	txp.AddData([]byte("tx_hash2"), &transaction.Transaction{Nonce: 2}, 2)
	...
	txp.MergeShardStores(1, 2)
	assert.Equal(t, 3, txp.ShardDataStore(2).Len(),
		"Mini pool for shard 1 should have 3 elements")
	assert.Nil(t, txp.ShardDataStore(1))
}

func TestShardedDataMoveData(t *testing.T) {
	t.Parallel()
	txp := shardedData.NewShardedData(nil)
	txp.AddData([]byte("tx_hash1"), &transaction.Transaction{Nonce: 1}, 1)
	txp.AddData([]byte("tx_hash2"), &transaction.Transaction{Nonce: 2}, 2)
	...
	assert.Equal(t, 2, txp.ShardDataStore(3).Len(),
		"Mini pool for shard 3 should have 2 elements")
}

func TestShardedDataRegisterAddedDataHandlerNilHandlerShouldIgnore(t *testing.T) {
	t.Parallel()

	sd := shardedData.NewShardedData(nil)
	sd.RegisterAddedDataHandler(nil)
