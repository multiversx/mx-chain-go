...
const aliceSeedHex = "413f42575f7f26fad3317a778771212fdb80245850981e48b58a4f25e344e8f9"
...
func TestConstructTransaction_NoDataNoValue(t *testing.T) {
	...
	tx.Signature = signTransaction(t, aliceSeedHex, tx)
...
func getPubkeyOfAddress(t *testing.T, address string) []byte {
	...
	pubkey, err := pubkeyConverter.Decode(address)
	require.Nil(t, err)
	return pubkey
}
...
func signTransaction(t *testing.T, senderSeedHex string, tx *Transaction) []byte {
	...
	jsonToSign := marshalTransactionToJSON(tx)
...
func marshalTransactionToJSON(tx *Transaction) []byte {
	frontendTransaction := &FrontendTransaction{
		Nonce:            tx.Nonce,
		Value:            tx.Value.String(),
		Receiver:         getAddressOfPubkey(tx.RcvAddr),
		Sender:           getAddressOfPubkey(tx.SndAddr),
	...
	jsonToSign, _ := marshalizer.Marshal(frontendTransaction)
	return jsonToSign
}

func marshalTransactionToProto(tx *Transaction) []byte {
	marshalizer := &marshal.GogoProtoMarshalizer{}
	data, _ := marshalizer.Marshal(tx)
	return data
}

func getAddressOfPubkey(pubkey []byte) string {
	pubkeyConverter, _ := pubkeyConverter.NewBech32PubkeyConverter(32)
	address := pubkeyConverter.Encode(pubkey)
	return address
}
