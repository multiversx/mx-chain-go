...
func TestAccumulate(t *testing.T) {
	...
	acc1 := new(big.Int).Exp(proofs[0], rsa.HashToPrime([]byte("one")), rsa.GetModulus())
	acc2 := new(big.Int).Exp(proofs[1], rsa.HashToPrime([]byte("two")), rsa.GetModulus())
	acc3 := new(big.Int).Exp(proofs[2], rsa.HashToPrime([]byte("three")), rsa.GetModulus())
	acc4 := new(big.Int).Exp(proofs[0], rsa.HashToPrime([]byte("four")), rsa.GetModulus())
	acc5 := new(big.Int).Exp(proofs[1], rsa.HashToPrime([]byte("three")), rsa.GetModulus())

	assert.Equal(t, acc.GetValue(), acc1)
	assert.Equal(t, acc.GetValue(), acc2)
	assert.Equal(t, acc.GetValue(), acc3)

	assert.NotEqual(t, acc.GetValue(), acc4)
	assert.NotEqual(t, acc.GetValue(), acc5)
