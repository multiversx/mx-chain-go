...
// PrivateKeyFromByteArrayRandom generates a private key given a randomly generated byte array
func (kg *keyGenerator) PrivateKeyFromByteArrayRandom(b []byte) (crypto.PrivateKey, error) {
	if len(b) == 0 {
		return nil, crypto.ErrInvalidParam
	}

	sc := kg.suite.CreateScalar()
	sc, err := sc.SetFromRandomBytes(b)
	if err != nil {
		return nil, err
	}
