...
func TestDataReplayer_Range_TransactionInPool(t *testing.T) {
	t.Parallel()
	...
	rwdTxMbHash := []byte("rwdTxMbHash")
	uTxMbHash := []byte("uTxMbHash")
	receiptMbHash := []byte("receiver")
	txHash := []byte("txHash")
	rwdTxHash := []byte("rwdTx")
	uTxHash := []byte("uTxHash")
	recHash := []byte("recHash")
	invalidMbHash := []byte("invalidMb")
	invalidTxHash := []byte("invalidTx")
	...
	sh0Hdr := &block.Header{
		...
		MiniBlockHeaders: []block.MiniBlockHeader{
			...
			{Type: block.RewardsBlock, SenderShardID: core.MetachainShardId, ReceiverShardID: 0, Hash: rwdTxMbHash},
			{Type: block.SmartContractResultBlock, SenderShardID: core.MetachainShardId, ReceiverShardID: 0, Hash: uTxMbHash},
			{Type: block.ReceiptBlock, SenderShardID: core.MetachainShardId, ReceiverShardID: 0, Hash: receiptMbHash},
			{Type: block.InvalidBlock, SenderShardID: core.MetachainShardId, ReceiverShardID: 0, Hash: invalidMbHash},
		},
	}
	sh0HdrBytes, _ := marshalizer.Marshal(sh0Hdr)
	shardHeadersPersister := mock.NewPersisterMock()
	...
	recMb := &block.MiniBlock{Type: block.ReceiptBlock, TxHashes: [][]byte{recHash}}
	recMbBytes, _ := marshalizer.Marshal(recMb)
	_ = miniBlocksPersister.Put(receiptMbHash, recMbBytes)

	invalidMb := &block.MiniBlock{Type: block.InvalidBlock, TxHashes: [][]byte{invalidTxHash}}
	invalidMbBytes, _ := marshalizer.Marshal(invalidMb)
	_ = miniBlocksPersister.Put(invalidMbHash, invalidMbBytes)
	...
	rec := &receipt.Receipt{}
	recBytes, _ := marshalizer.Marshal(rec)

	invalidTx := &transaction.Transaction{}
	invalidTxBytes, _ := marshalizer.Marshal(invalidTx)

	_ = txPersister.Put(txHash, txBytes)
	_ = txPersister.Put(rwdTxHash, rwdTxBytes)
	_ = txPersister.Put(uTxHash, uTxBytes)
	_ = txPersister.Put(recHash, recBytes)
	_ = txPersister.Put(invalidTxHash, invalidTxBytes)
	...
	args.GeneralConfig.UnsignedTransactionStorage.DB.FilePath = "UnsignedTransactions"
	args.GeneralConfig.TxStorage.DB.FilePath = "Transactions"
	args.GeneralConfig.RewardTxStorage.DB.FilePath = "RewardTransactions"
	args.GeneralConfig.MiniBlocksStorage.DB.FilePath = "MiniBlocks"
	args.GeneralConfig.BlockHeaderStorage.DB.FilePath = "BlockHeaders"
	args.ShardCoordinator = &mock.ShardCoordinatorMock{ShardID: 0, NumOfShards: 1}
	realHeaderMarshalizer, _ := databasereader.NewHeaderMarshalizer(marshalizer)
	args.HeaderMarshalizer = realHeaderMarshalizer
	dr, _ := dataprocessor.NewDataReplayer(args)
