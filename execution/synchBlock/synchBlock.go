...
type bootstrap struct {
	...
	requestedHeaderNounce uint64
	chRcvHdr              chan bool

	requestedBodyNounce uint64
	chRcvBdy            chan bool
...
func (bs *bootstrap) synchBlock() error {
	...
	if bs.hdr == nil {
		bs.hdr = bs.getHeaderFromPool(nounce)
	...
	if bs.blk == nil {
		bs.blk = bs.getBodyFromPool(nounce)
	...
	err := bs.blkExecutor.ProcessBlock(bs.blkc, bs.hdr, bs.blk)
...
func (bs *bootstrap) requestHeader(nounce uint64) {
	if bs.OnRequestHeader != nil {
		bs.requestedHeaderNounce = nounce
...
func (bs *bootstrap) receivedHeader(nounce uint64) {
	if bs.requestedHeaderNounce == nounce {
		bs.requestedHeaderNounce = 0
		bs.chRcvHdr <- true
	}
}
...
func (bs *bootstrap) receivedBody(nounce uint64) {
	if bs.requestedBodyNounce == nounce {
		bs.requestedBodyNounce = 0
		bs.chRcvBdy <- true
	}
}
