...
// ArgsNewRequestersContainerFactory defines the arguments for the requestersContainerFactory constructor
type ArgsNewRequestersContainerFactory struct {
	ShardCoordinator       sharding.Coordinator
	Messenger              dataRetriever.TopicMessageHandler
	Marshaller             marshal.Marshalizer
...
func NewRequestersContainerFactory(args ArgsNewRequestersContainerFactory) (*requestersContainerFactory, error) {
	...
	if check.IfNil(args.ExistingRequesters) {
		return nil, fmt.Errorf("%w for requesters", update.ErrNilContainer)
	}
